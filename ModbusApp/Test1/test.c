...
static void usb_uart_serial_init(Uart* usb_uart, uint8_t uart_ch, uint32_t br) {
    furi_assert(!usb_uart->serial_handle);
    usb_uart->serial_handle = furi_hal_serial_control_acquire(uart_ch);
    furi_assert(usb_uart->serial_handle);

    furi_hal_serial_init(usb_uart->serial_handle, br);
...
static int32_t uart_worker(void* context) {
    ...
    while(1) {
        ...
        if(events & WorkerEvtRxDone) {
            size_t len = furi_stream_buffer_receive(app->uart->rxStream, app->uart->rxBuff, RX_BUF_SIZE, 0);
            if(len > 0) {
...
Uart* Uart_Alloc(void* context) {
    ...
    uint32_t BR = atoi(baudrateValues[app->config->baudrate]);
    usb_uart_serial_init(uart, UART_CH, BR);
...
void usb_uart_set_config(Config* cfg, Config* newcfg) {
    furi_assert(cfg);
    furi_assert(newcfg);
    memcpy(cfg, newcfg, sizeof(Config));
