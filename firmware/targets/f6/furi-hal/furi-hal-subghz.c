...
static void furi_hal_subghz_async_tx_refill(uint32_t* buffer, size_t samples) {
    while (samples > 0) {
        ...
        furi_assert((samples % 2) ? !level_duration_get_level(ld) : level_duration_get_level(ld));
    ...
    if(LL_TIM_IsActiveFlag_UPDATE(TIM2)) {
        ...
        if (LL_TIM_GetAutoReload(TIM2) == 0) {
            if (furi_hal_subghz_state == SubGhzStateAsyncTx) {
                ...
                //hal_gpio_init(&gpio_cc1101_g0, GpioModeOutputPushPull, GpioPullDown, GpioSpeedLow); //todo check for carrier at the end of the parcel
            }
        }
    }
}
