...
void api_hal_rfid_pins_read() {
    ...
    hal_gpio_init_alt(&gpio_rfid_carrier_out, GpioModeOutputPushPull, GpioSpeedLow, GpioPullNo, GpioAltFn1TIM1);
...
void api_hal_rfid_tim_read(float freq, float duty_cycle) {
    uint32_t period = (uint32_t)((SystemCoreClock / (LFRFID_TIM.Init.Prescaler + 1)) / freq) - 1;
    ...
    if(HAL_TIM_OC_Init(&LFRFID_TIM) != HAL_OK) {
        Error_Handler();
    }
