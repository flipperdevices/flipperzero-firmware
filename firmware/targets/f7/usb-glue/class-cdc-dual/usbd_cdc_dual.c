...
static uint8_t USBD_CDC_Init_Single(USBD_HandleTypeDef* pdev, uint8_t cfgidx);
static uint8_t USBD_CDC_Init_Dual(USBD_HandleTypeDef* pdev, uint8_t cfgidx);
static uint8_t USBD_CDC_DeInit_Single(USBD_HandleTypeDef* pdev, uint8_t cfgidx);
static uint8_t USBD_CDC_DeInit_Dual(USBD_HandleTypeDef* pdev, uint8_t cfgidx);
static uint8_t USBD_CDC_Setup(USBD_HandleTypeDef* pdev, USBD_SetupReqTypedef* req);
static uint8_t USBD_CDC_DataIn(USBD_HandleTypeDef* pdev, uint8_t epnum);
static uint8_t USBD_CDC_DataOut(USBD_HandleTypeDef* pdev, uint8_t epnum);
static uint8_t USBD_CDC_EP0_RxReady(USBD_HandleTypeDef* pdev);

static uint8_t* USBD_CDC_GetFSCfgDesc_Single(uint16_t* length);
static uint8_t* USBD_CDC_GetFSCfgDesc_Dual(uint16_t* length);
...
USBD_ClassTypeDef USBD_CDC_Single = {
    USBD_CDC_Init_Single,
    USBD_CDC_DeInit_Single,
    USBD_CDC_Setup,
    NULL, /* EP0_TxSent, */
...
USBD_ClassTypeDef USBD_CDC_Dual = {
    ...
    NULL,
    NULL,
    USBD_CDC_GetFSCfgDesc_Dual,
    NULL,
    USBD_CDC_GetDeviceQualifierDescriptor,
};

/* USB CDC device Configuration Descriptor - Single mode */
__ALIGN_BEGIN static uint8_t USBD_CDC_Single_CfgFSDesc[USB_CDC_SINGLE_CONFIG_DESC_SIZ] __ALIGN_END = {
    /* Configuration Descriptor */
    0x09, /* bLength: Configuration Descriptor size */
    USB_DESC_TYPE_CONFIGURATION, /* bDescriptorType: Configuration */
    USB_CDC_SINGLE_CONFIG_DESC_SIZ, /* wTotalLength:no of returned bytes */
    0x00,
...
/* USB CDC device Configuration Descriptor - Dual mode*/
__ALIGN_BEGIN static uint8_t USBD_CDC_Dual_CfgFSDesc[USB_CDC_DUAL_CONFIG_DESC_SIZ] __ALIGN_END = {
    /* Configuration Descriptor */
    0x09, /* bLength: Configuration Descriptor size */
    USB_DESC_TYPE_CONFIGURATION, /* bDescriptorType: Configuration */
    USB_CDC_DUAL_CONFIG_DESC_SIZ, /* wTotalLength:no of returned bytes */
    0x00,
...
static uint8_t USBD_CDC_Init_Dual(USBD_HandleTypeDef* pdev, uint8_t cfgidx) {
    ...
    /* Prepare Out endpoint to receive next packet 1 */
    (void)USBD_LL_PrepareReceive(pdev, CDC_1_OUT_EP, hcdc->RxBuffer, CDC_DATA_FS_OUT_PACKET_SIZE);
    /* Prepare Out endpoint to receive next packet 2 */
    (void)USBD_LL_PrepareReceive(pdev, CDC_2_OUT_EP, hcdc->RxBuffer, CDC_DATA_FS_OUT_PACKET_SIZE);

    return (uint8_t)USBD_OK;
}

/**
  * @brief  USBD_CDC_DeInit_Single
...
static uint8_t* USBD_CDC_GetFSCfgDesc_Single(uint16_t* length) {
    ...
    *length = (uint16_t)sizeof(USBD_CDC_Single_CfgFSDesc);
    return USBD_CDC_Single_CfgFSDesc;
}
...
static uint8_t* USBD_CDC_GetFSCfgDesc_Dual(uint16_t* length) {
    ...
    *length = (uint16_t)sizeof(USBD_CDC_Dual_CfgFSDesc);
