...
void hid_svc_start() {
    ...
    status = aci_gatt_update_char_value_(
...
bool hid_svc_update_report_map(const uint8_t* data, uint16_t len) {
    ...
    tBleStatus status = aci_gatt_update_char_value_(
...
static bool hid_svc_update_info_(uint8_t* data, uint16_t len) {
    furi_assert(data);
    furi_assert(hid_svc);

    tBleStatus status =
        aci_gatt_update_char_value_(hid_svc->svc_handle, hid_svc->info_char_handle, 0, len, data);
    if(status) {
...
bool hid_svc_update_info(uint8_t* data) {
    return hid_svc_update_info_(data, 4);
}
