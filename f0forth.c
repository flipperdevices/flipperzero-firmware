...
{
	gui_remove_view_port(f0f_state->view_dispatcher->gui,
			f0f_state->view_dispatcher->view_port);
	f0f_state->view_dispatcher->gui = NULL;
	furi_record_close(RECORD_GUI);
}

static void f0forth_bgloader_loop(F0ForthState *f0f_state, const char
		*bg_app_path)
{
	while (true) {
...
int32_t f0forth(const char *args)
{
	...
	f0f_state->orig_input_cb =
		f0f_state->view_dispatcher->view_port->input_callback;
	view_port_input_callback_set(f0f_state->view_dispatcher->view_port,
			f0forth_hooked_input_callback, f0f_state);
