...
#define KASAN_TAG_INVALID 0xFF
#define KASAN_KMALLOC_REDZONE 0xAA

void kasan_poison_shadow(void *addr, size_t size, uint8_t tag){
    UNUSED(addr);
    UNUSED(size);
    UNUSED(tag);
}

void kasan_unpoison_shadow(void *addr, size_t size){
    UNUSED(addr);
    UNUSED(size);
}
