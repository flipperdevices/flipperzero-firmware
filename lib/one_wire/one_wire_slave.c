...
uint32_t onewire_slave_wait_while_gpio_is(OneWireSlave* bus, uint32_t time, const bool pin_value) {
    ...
    do {
        ...
        if(furi_hal_onewire_pin_get_level(bus->gpio_pin) != pin_value) {
...
bool onewire_slave_show_presence(OneWireSlave* bus) {
    ...
    // show presence
    furi_hal_onewire_pin_low(bus->gpio_pin);
    furi_delay_us(OWS_PRESENCE_MIN);
    furi_hal_onewire_pin_high(bus->gpio_pin);
...
bool onewire_slave_bus_start(OneWireSlave* bus) {
    ...
    if(bus->device == NULL) {
        ...
        FURI_CRITICAL_ENTER();
        furi_hal_onewire_start_drive_in_isr(bus->gpio_pin);
        bus->error = NO_ERROR;
        ...
        furi_hal_onewire_start_interrupt_in_isr(bus->gpio_pin);
        FURI_CRITICAL_EXIT();
    }
...
static void exti_cb(void* context) {
    ...
    volatile bool input_state = furi_hal_onewire_pin_get_level(bus->gpio_pin);
...
void onewire_slave_start(OneWireSlave* bus) {
    furi_hal_onewire_add_interrupt(bus->gpio_pin, exti_cb, bus);
    furi_hal_onewire_start_interrupt(bus->gpio_pin);
}

void onewire_slave_stop(OneWireSlave* bus) {
    UNUSED(bus);
    furi_hal_onewire_stop(bus->gpio_pin);
    furi_hal_onewire_remove_interrupt(bus->gpio_pin);
}
