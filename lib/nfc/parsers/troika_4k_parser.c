...
bool troika_4k_parser_read(NfcWorker* nfc_worker, FuriHalNfcTxRxContext* tx_rx) {
    ...
    for(size_t i = 0; i < COUNT_OF(troika_4k_keys); i++) {
        mf_classic_set_key_found(
            mf_classic_data, troika_4k_keys[i].sector, MfClassicKeyA, troika_4k_keys[i].key_a);
        mf_classic_set_key_found(
            mf_classic_data, troika_4k_keys[i].sector, MfClassicKeyB, troika_4k_keys[i].key_b);
    }
