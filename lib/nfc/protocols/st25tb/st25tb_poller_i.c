...
St25tbError
    ...
    do {
        ...
        FURI_LOG_D(TAG, "read(%d) result: %08lX", block_number, *block);
    } while(false);
...
St25tbError
    ...
    do {
        ...
        ret = st25tb_poller_async_read_block(instance, &block_check, block_number);
        if(ret != St25tbErrorNone) {
        ...
        FURI_LOG_D(TAG, "write block %d ok", block_number);
    } while(false);
