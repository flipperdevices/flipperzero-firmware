...
/// @brief Generic Felica request same for both read and write operations.
typedef struct {
    uint8_t length;
    FelicaCommandHeader header;
} FelicaListenerGenericRequest;

/// @brief Generic request but with list of requested elements.
typedef struct {
...
/// @brief Struct for write request data. All data are placed right after @ref FelicaListenerRequest data.
typedef struct {
    FelicaBlockData blocks[FELICA_LISTENER_WRITE_BLOCK_COUNT_MAX];
} FelicaListenerWriteBlockData;

/// @brief Write command handler signature. Depending on requested block write behaviour can be different,
/// therefore different handlers are used.
typedef void (*FelicaCommandWriteBlockHandler)(
    FelicaListener* instance,
    const uint8_t block_number,
    const FelicaBlockData* data_block);

/// @brief Read command handler signature. Depending on requested block read behaviour can be different,
/// therefore different handlers are used.
...
/// @brief Resets card authentication state after field off, also resets session key and perform
/// post process operations with some blocks.
/// @param instance pointer to the listener instance to be used.
void felica_listener_reset(FelicaListener* instance);

/// @brief Performs WCNT increasing in case of write operation.
/// @param data pointer to Felica card data.
void felica_wcnt_increment(FelicaData* data);

/// @brief Compares IDm of card loaded for emulation with IDm from request.
/// @param instance pointer to the listener instance to be used.
...
/// If block list element amount is invalid or request data is missing, such request will be
/// ignored.
/// @param instance pointer to the listener instance to be used.
...
/// @brief Used to take first element from block element list. Each element in block list can be 2 or
/// 3-bytes length and they can be mixed together. Therefore before returning the element,
/// function checks whether there is enough bytes in the request for this element, in order to
/// avoid memory casting behind the request block.
/// @param instance pointer to the listener instance to be used.
/// @param request pointer to received request.
/// @return Pointer to the first element of the list or NULL if there is not enough bytes in the request.
...
/// @brief Calculates pointer to data blocks in case of write operation, because block list elements size can
/// vary. For calculation it uses previously calculated block list size and FelicaListenerGenericRequest size.
/// @param instance pointer to the listener instance to be used.
...
/// @brief Function validates write request data and sets Felica SF1 and SF2 flags directly to the response.
/// When something is wrong with data in the request (for example non-existing block is requested) this function
/// sets proper SF1 and SF2 flags which will be then send to reader. When every thing is fine SF1 and SF2 are 0.
/// Also this function performs authentiocation checks by MAC_A calculation if request contains MAC_A.
/// @param instance pointer to the listener instance to be used.
/// @param request pointer to received write request.
/// @param data pointer to data which request wants to write.
/// @param response pointer to the response to where SF1 and SF2 flags will be populated.
/// @return True if request is fine also SF1 and SF2 will be 0. False if something is wrong and SF1, SF2 will provide
...
//In such case there will be no any data
/// in the request, only flags. In case when request is fine SF1 and SF2 will be 0 and data will be populated
/// to the

/// @brief Function validates read request data and sets Felica SF1 and SF2 flags directly to the response.
/// When something is wrong with data in the request (for example non-existing block is requested) this function
/// sets proper SF1 and SF2 flags which will be then send to reader. In such case there will be no any data
/// in the request, only flags. In case when request is fine SF1 and SF2 will be 0 and data will be populated
/// to the response after them. Attention! This function doesn't populate any data, it only allows further
/// processing where those data will be populated.
/// @param instance pointer to the listener instance to be used.
/// @param request pointer to received write request.
/// @param resp_header pointer to the response to where SF1 and SF2 and data flags will be populated.
/// @return True if request is fine also SF1 and SF2 will be 0. False if something is wrong and SF1, SF2 will provide
/// more detailed information about the error.
...
/// @brief Function returns appropiate block handler for processing read operation depending on number of block.
...
/// @brief Function returns appropiate block handler for processing write operation depending on number of block.
/// @param block_number number of block.
/// @return pointer to block handler function.
