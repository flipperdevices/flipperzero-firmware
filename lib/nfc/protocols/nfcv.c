...
ReturnCode nfcv_read_blocks(
    ...
    for(size_t block = 0; block < nfcv_data->block_num; block++) {
        ...
        memcpy(&(nfcv_data->data[block * nfcv_data->block_size]), &rxBuf[1], nfcv_data->block_size);
        FURI_LOG_D(TAG, "  %02X %02X %02X %02X",
            nfcv_data->data[block * nfcv_data->block_size + 0], nfcv_data->data[block * nfcv_data->block_size + 1],
            nfcv_data->data[block * nfcv_data->block_size + 2], nfcv_data->data[block * nfcv_data->block_size + 3]);
    }
...
ReturnCode nfcv_read_sysinfo(FuriHalNfcDevData* nfc_data, NfcVData* nfcv_data) {
    ...
    if(ret == ERR_NONE) {
        ...
        FURI_LOG_D(TAG, "  DSFID %d, AFI %d, Blocks %d, Size %d, IC Ref %d",
            nfcv_data->dsfid, nfcv_data->afi,
            nfcv_data->block_num, nfcv_data->block_size, nfcv_data->ic_ref);
        return ret;
    }
...
void nfcv_emu_alloc(NfcVData* nfcv_data) {
    ...
    if(!nfcv_data->emu_air.nfcv_resp_unmod_256) {
        ...
        nfcv_data->emu_air.nfcv_resp_unmod_256->edge_timings[0] = (uint32_t)(NFCV_RESP_SUBC1_UNMOD_256 * DIGITAL_SIGNAL_UNIT_S);
    ...
    if(!nfcv_data->emu_air.nfcv_resp_pulse_32) {
        ...
        nfcv_data->emu_air.nfcv_resp_pulse_32->edge_timings[0] = (uint32_t)(NFCV_RESP_SUBC1_PULSE_32 * DIGITAL_SIGNAL_UNIT_S);
        nfcv_data->emu_air.nfcv_resp_pulse_32->edge_timings[1] = (uint32_t)(NFCV_RESP_SUBC1_PULSE_32 * DIGITAL_SIGNAL_UNIT_S);
    ...
    if(!nfcv_data->emu_air.nfcv_resp_one) {
        ...
        digital_signal_append(nfcv_data->emu_air.nfcv_resp_one, nfcv_data->emu_air.nfcv_resp_unmod_256);
    ...
    if(!nfcv_data->emu_air.nfcv_resp_zero) {
        ...
        for(size_t i = 0; i < 8; i++) {
            digital_signal_append(nfcv_data->emu_air.nfcv_resp_zero, nfcv_data->emu_air.nfcv_resp_pulse_32);
        }
        digital_signal_append(nfcv_data->emu_air.nfcv_resp_zero, nfcv_data->emu_air.nfcv_resp_unmod_256);
    }
    if(!nfcv_data->emu_air.nfcv_resp_sof) {
    ...
    digital_sequence_set_signal(nfcv_data->emu_air.nfcv_signal, NFCV_SIG_SOF, nfcv_data->emu_air.nfcv_resp_sof);
    digital_sequence_set_signal(nfcv_data->emu_air.nfcv_signal, NFCV_SIG_BIT0, nfcv_data->emu_air.nfcv_resp_zero);
    digital_sequence_set_signal(nfcv_data->emu_air.nfcv_signal, NFCV_SIG_BIT1, nfcv_data->emu_air.nfcv_resp_one);
    digital_sequence_set_signal(nfcv_data->emu_air.nfcv_signal, NFCV_SIG_EOF, nfcv_data->emu_air.nfcv_resp_eof);
}
...
void nfcv_emu_handle_packet(FuriHalNfcTxRxContext* tx_rx, FuriHalNfcDevData* nfc_data, void* nfcv_data_in) {
    ...
    switch(ctx->command) {
        case ISO15693_INVENTORY: {
            ...
            nfcv_emu_send(tx_rx, nfcv_data, ctx->response_buffer, 10, ctx->response_flags, ctx->send_time);
        ...
        case ISO15693_SELECT: {
            ctx->response_buffer[0] = ISO15693_NOERROR;
    ctx->send_time = nfcv_data->eof_timestamp + NFCV_FDT_FC(4300);
            nfcv_emu_send(tx_rx, nfcv_data, ctx->response_buffer, 1, ctx->response_flags, ctx->send_time);
        ...
        case ISO15693_READBLOCK: {
            ...
            if(block + blocks > nfcv_data->block_num) {
                ...
                nfcv_emu_send(tx_rx, nfcv_data, ctx->response_buffer, 1 + nfcv_data->block_size * blocks, ctx->response_flags, ctx->send_time);
            }
        ...
        case ISO15693_WRITEBLOCK: {
            ...
            if(block + blocks > nfcv_data->block_num || ctx->payload_offset + data_len + 2 > nfcv_data->frame_length) {
                ctx->response_buffer[0] = ISO15693_ERROR_CMD_NOT_REC;
            } else {
                ctx->response_buffer[0] = ISO15693_NOERROR;
                memcpy(&nfcv_data->data[nfcv_data->block_size * block], &nfcv_data->frame[ctx->payload_offset + data_pos], data_len);
            }
            nfcv_emu_send(tx_rx, nfcv_data, ctx->response_buffer, 1, ctx->response_flags, ctx->send_time);
        ...
        case ISO15693_GET_SYSTEM_INFO: {
            ...
            nfcv_emu_send(tx_rx, nfcv_data, ctx->response_buffer, 15, ctx->response_flags, ctx->send_time);
