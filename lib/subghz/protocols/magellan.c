...
static void subghz_protocol_magellan_get_event_serialize(uint8_t event, string_t output) {
    string_cat_printf(
        output,
        "%s%s%s%s%s%s%s%s",
...
void subghz_protocol_decoder_magellan_get_string(void* context, string_t output) {
    furi_assert(context);
    SubGhzProtocolDecoderMagellan* instance = context;
