...
bool subghz_tx_rx_worker_rx(SubGhzTxRxWorker* instance, uint8_t* data, uint8_t* size) {
    ...
    while(furi_hal_gpio_read(subghz_g0_pin)) {
        furi_delay_tick(1);
        if(!--timeout) {
...
void subghz_tx_rx_worker_tx(SubGhzTxRxWorker* instance, uint8_t* data, size_t size) {
    ...
    while(!furi_hal_gpio_read(subghz_g0_pin)) { // Wait for GDO0 to be set -> sync transmitted
        furi_delay_tick(1);
        if(!--timeout) {
    ...
    while(furi_hal_gpio_read(subghz_g0_pin)) { // Wait for GDO0 to be cleared -> end of packet
        furi_delay_tick(1);
        if(!--timeout) {
            FURI_LOG_W(TAG, "TX cc1101_g0 timeout");
            break;
