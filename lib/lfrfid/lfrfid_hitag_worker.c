...
#define DMA_BUFFER_SIZE 4000	//I need to keep this big enough? to have enough time inbetween HT & TC events perhaps?
...
void lfrfid_hitag_worker_carrier_out_start(
	...
	TIM_InitStruct.Prescaler = SystemCoreClock / (125000 * HITAG_BASEPERIOD) - 1;	// sets the basis TIMER frequency to 8*freq (1MHz) for the timer
    TIM_InitStruct.Autoreload = HITAG_BASEPERIOD - 1;	//initial PWM period =125kHz  (ARR will be handled via DMA further on)
	...
	TIM_OC_InitStruct.CompareValue = HITAG_BASEPERIOD/2;	//initial pulse duration of half period ((CCR will be handled via DMA further on)
    LL_TIM_OC_Init(
        CARRIER_OUT_TIMER, CARRIER_OUT_TIMER_CHANNEL, &TIM_OC_InitStruct);

    LL_TIM_OC_SetPolarity(
        CARRIER_OUT_TIMER, CARRIER_OUT_TIMER_CHANNEL, LL_TIM_OCPOLARITY_HIGH);
    ...
    furi_hal_interrupt_set_isr(FuriHalInterruptIdDma1Ch1, lfrfid_hitag_worker_carrier_out_dma_isr, context);
...
static void lfrfid_hitag_worker_pull_out_dma_start(size_t length){
	...
	//set OC to PWM1 mode
	LL_TIM_OC_SetMode(PULL_OUT_TIMER, PULL_OUT_TIMER_CHANNEL, LL_TIM_OCMODE_PWM1);			//during emulate (dma controlled) mode
...
static void lfrfid_hitag_worker_pull_out_dma_setup(uint32_t* duration, uint32_t* pulse, size_t length, void* capture_context){
    ...
    furi_hal_interrupt_set_isr(PULL_OUT_DMA_CH1_IRQ, lfrfid_hitag_worker_pull_out_dma_isr, capture_context);
...
static void lfrfid_hitag_worker_carrier_in_isr(void* capture_context) {
    if(LL_TIM_IsActiveFlag_CC1(CARRIER_IN_TIMER)) {
		...
		buffer_stream_send_from_isr(capData->stream, varint_pair_get_data(capData->pair), varint_pair_get_size(capData->pair));
		varint_pair_reset(capData->pair);
...
static void lfrfid_hitag_worker_carrier_in_ETR_mode(void* capture_context, uint8_t ext_prescaler){
	...
	LL_TIM_GenerateEvent_UPDATE(CARRIER_IN_TIMER); //prescaler is only applied at next update event //TODO: how to prevent this from creating a capdata entry
	...
	furi_hal_gpio_init_ex(&gpio_rfid_carrier, GpioModeAltFunctionPushPull, GpioPullNo, GpioSpeedLow, GpioAltFn2TIM2);
...
static void lfrfid_hitag_worker_carrier_in_start(void* capture_context, uint8_t ext_prescaler, uint32_t* duration, uint32_t* pulse, size_t length) {
	FURI_CRITICAL_ENTER();
    ...
    TIM_InitStruct_Ref.Prescaler = 64 - 1;	//system base freq is 64MHz, so this sets base freq for TIM to 1MHz (aka 1us period)
    ...
    LL_TIM_InitTypeDef TIM_InitStruct = {0};
    TIM_InitStruct.Prescaler = 64 - 1;	//system base freq is 64MHz, so this sets base freq for TIM to 1MHz (aka 1us period)
    ...
    LL_TIM_SetClockSource(CARRIER_IN_TIMER, LL_TIM_CLOCKSOURCE_INTERNAL);	//default is internal, so likely not required
    ...
    LL_TIM_IC_SetActiveInput(CARRIER_IN_TIMER, CARRIER_IN_TIMER_DIR_CH, LL_TIM_ACTIVEINPUT_DIRECTTI);
	...
	*/
    ...
    furi_hal_interrupt_set_isr(FuriHalInterruptIdTIM2, lfrfid_hitag_worker_carrier_in_isr, capture_context);
	
	//OUTPUT COMPARE SETUP
	LL_TIM_OC_InitTypeDef TIM_OC_InitStruct = {0};
    //TIM_OC_InitStruct.OCMode = LL_TIM_OCMODE_PWM1;			//during emulate (dma controlled) mode
	TIM_OC_InitStruct.OCMode = LL_TIM_OCMODE_FORCED_INACTIVE;	//during carrier in put output to forced low state
    TIM_OC_InitStruct.OCState = LL_TIM_OCSTATE_ENABLE;
    TIM_OC_InitStruct.CompareValue = 0;	//0% this should have almost similar effect as keeping output forced inactive (there's still some micropulse emited, going high at ARR and immediately down again at CCR value)
    LL_TIM_OC_Init(PULL_OUT_TIMER, PULL_OUT_TIMER_CHANNEL, &TIM_OC_InitStruct);
	LL_TIM_OC_SetPolarity(
        PULL_OUT_TIMER, PULL_OUT_TIMER_CHANNEL, LL_TIM_OCPOLARITY_HIGH);	//active high (gpio goes high when pulse is high)
...
static void lfrfid_hitag_worker_yield_dma_buffer_BPLM(LfRfidHitagCMDData* dataCMD, LfRfidHitagCMDData* backupCMD, TimerDMAData* dataDMA, size_t start, size_t len){
	...
	while(1){
		...
		if (data->CMDposition == data->CMDlength){
			data->CMDcount++;
			if (dataCMD->CMDposition < dataCMD->CMDlength){	//currently backup & new unprocessed command available --> switch over to real CMD
				data->CMDactive = false;
				data = dataCMD;
				data->CMDactive = true;
				data->CMDcount = 0;
			} else if (dataCMD->CMDloop){	//if looping (real or backup) --> reset real (current) CMD to startposition
				data->CMDposition = 0;
				data->CMDsubposition = 0;
			} else if (dataCMD->CMDactive){	//real command and not looping --> switch to backup CMD and reset backup CMD to startposition
				data->CMDactive = false;
				data = backupCMD;
				data->CMDactive = true;
				data->CMDposition = 0;
		...
		switch (data->CMDtype[data->CMDposition])
		{
			...
			case HITAG_OFF:		//x OFF cycles (update previous arr value)
				if(s>start){
					dataDMA->timer_buffer_arr[s-1]+=data->CMDvalue[data->CMDposition]*HITAG_BASEPERIOD;
				}
				break;
...
static uint16_t lfrfid_hitag_worker_yield_dma_buffer_MC(LfRfidHitagReplyData* dataReply, TimerDMAData* dataDMA, uint8_t carrierPrescaler){
...
static uint16_t lfrfid_hitag_worker_yield_dma_buffer_AC(LfRfidHitagReplyData* dataReply, TimerDMAData* dataDMA, uint8_t carrierPrescaler){
	//based on external carrier with freq of 125kHz (period of 8us)
...
static bool lfrfid_hitag_worker_validate_bits(uint32_t* bits, uint8_t bitcounter){
	if (47 >= bitcounter && bitcounter >= 45){
        for (uint8_t i=0;i<bitcounter-44;i++){
            ...
            if (lfrfid_hitag_worker_validate_command(bits, 45))
				return true;
		}
	}
        ...
        for (uint8_t i=0;i<bitcounter-19;i++){
            ...
            if (lfrfid_hitag_worker_validate_command(bits, 20))
				return true;
		}
	}
        ...
        for (uint8_t i=0;i<bitcounter-4;i++){
            ...
            if (lfrfid_hitag_worker_validate_command(bits, 5))
				return true;
		}
	}
    return false;
...
static uint8_t lfrfid_hitag_worker_validate_ripple_delta(uint32_t delta){
	if (18 <= delta && delta <= 22)
        return 1;
    if (26 <= delta && delta <= 32)
        return 1;
    return 0;
}
...
static int32_t lfrfid_hitag_worker_emulate_thread(void* thread_context) {
	...
	lfrfid_hitag_worker_carrier_in_start(capData, worker->carrierPrescaler, dataDMA->timer_buffer_arr, dataDMA->timer_buffer_ccr, dmaLen);
	...
	while(1) {
		...
		if(buffer != NULL) {
			...
			while(index < buffsize) {
				...
				size_t tmp_size=varint_uint32_unpack(&duration, &buffdata[index], buffsize - index);
				...
				if(tmp_size==0) {
					...
					if (!carrier_on){   //detect if carrier is available
						...
						if (carrier_on){//if carrier is still available
							...
							if (duration > prevduration && duration > period){
							...
							} else  if (duration < prevduration && localMax > period && periodcounter <= localMaxIndex+2) {
								...
								if (miniMaxDelta > 0){
									...
									if ((lfrfid_hitag_worker_validate_ripple_delta(prevMaxDelta+miniMaxDelta) + lfrfid_hitag_worker_validate_ripple_delta(maxDelta-miniMaxDelta)) > (lfrfid_hitag_worker_validate_ripple_delta(prevMaxDelta) + lfrfid_hitag_worker_validate_ripple_delta(maxDelta))){
										...
										if (maxBitCounter > 0){
											maxBits[(maxBitCounter-1)/32] |= (one >> (maxBitCounter-1)%32);
										}
								...
								if (maxDelta >= 18){
									if (18 <= maxDelta && maxDelta <= 22){
										...
										maxBits[(maxBitCounter)/32] |= (one >> (maxBitCounter)%32);
										maxBitCounter++;
							...
							if (periodcounter == lastMaxIndex + 40/prescaler && maxBitCounter >0){
							...
							if (duration < prevduration && duration < period){
							...
							} else  if (duration > prevduration && localMin < period && periodcounter <= localMinIndex+2) {
								...
								if (miniMinDelta > 0){
									...
									if ((lfrfid_hitag_worker_validate_ripple_delta(prevMinDelta+miniMinDelta) + lfrfid_hitag_worker_validate_ripple_delta(minDelta-miniMinDelta)) > (lfrfid_hitag_worker_validate_ripple_delta(prevMinDelta) + lfrfid_hitag_worker_validate_ripple_delta(minDelta))){
										//update the previous (last) bit from 0 to 1 if a command was ongoing
										if (minBitCounter > 0){
											minBits[(minBitCounter-1)/32] |= (one >> (minBitCounter-1)%32);
										}
								...
								if (minDelta >= 18){
									if (18 <= minDelta && minDelta <= 22){
										...
										minBits[(minBitCounter)/32] |= (one >> (minBitCounter)%32);
										minBitCounter ++;
							...
							if (periodcounter == lastMinIndex + 40/prescaler && minBitCounter >0){
								if (lfrfid_hitag_worker_validate_bits(minBits, minBitCounter)){
									cmd = -1;
									cmdIndex = localMinIndex;
								...
								if (size == 5 && (bits[0]>>(32-5)) == 0b11001){			//SET_CCNEW
									...
									if (SN_cmd == SN_tag){
										...
										if (tagMode == HitagModeBasic){
											...
											dataReply->replyvalue[1] = worker->tag->pageData[4+0];
											dataReply->replytype[1] = HITAG_BYTE;
											dataReply->replyvalue[2] = worker->tag->pageData[4+1];
											dataReply->replytype[2] = HITAG_BYTE;
											dataReply->replyvalue[3] = worker->tag->pageData[4+2];
											dataReply->replytype[3] = HITAG_BYTE;
											dataReply->replyvalue[4] = worker->tag->pageData[4+3];
											dataReply->replytype[4] = HITAG_BYTE;
											...
											uint8_t crc = CRC_PRESET;
											lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[1], 8);
											lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[2], 8);
											lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[3], 8);
											lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[4], 8);
										...
										dmaLen = lfrfid_hitag_worker_yield_dma_buffer_MC(dataReply, dataDMA, worker->carrierPrescaler);
									}										
								} 
									...
									if (tagState == HitagStateSelected){
										...
										switch (rwCmd){
											case (0b1100):
												//read public page
												if (worker->tag->pageKnown[addr] && worker->tag->pagePublic[addr]){
													pages = 1;
												}
												break;
											case (0b1101):
												...
												for (uint8_t p=addr;p<addr+pages;p++){
													if (!worker->tag->pageKnown[p] || !worker->tag->pagePublic[p]){
														pages=0;
														break;
													}
												}
										...
										if (pages>0){
											...
											for (uint8_t i=0;i<pages;i++){
												dataReply->replyvalue[1+i*4+0] = worker->tag->pageData[(addr+i)*4+0];
												dataReply->replytype[1+i*4+0] = HITAG_BYTE;
												dataReply->replyvalue[1+i*4+1] = worker->tag->pageData[(addr+i)*4+1];
												dataReply->replytype[1+i*4+1] = HITAG_BYTE;
												dataReply->replyvalue[1+i*4+2] = worker->tag->pageData[(addr+i)*4+2];
												dataReply->replytype[1+i*4+2] = HITAG_BYTE;
												dataReply->replyvalue[1+i*4+3] = worker->tag->pageData[(addr+i)*4+3];
											...
											if (tagMode == HitagModeBasic){
												...
												for (uint8_t i=0;i<pages;i++){
													lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[1+i*4+0], 8);
													lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[1+i*4+1], 8);
													lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[1+i*4+2], 8);
													lfrfid_hitag_worker_calc_crc(&crc, dataReply->replyvalue[1+i*4+3], 8);
												}
	...
	while(1) {
		...
		if(buffer != NULL) {
			...
			while(expectedBits > 0 && index < buffsize) {
				...
				if(!varint_pair_unpack(&buffdata[index], buffsize - index, &pulse, &duration, &tmp_size)) {
					FURI_LOG_E(TAG, "can't unpack varint pair");
					break;
				} else {
					index += tmp_size;
						...
						if (bits == expectedBits){//eligable bitsequence detected, accept it only after 3 consequtive readings
							...
							if (readings == 0){
								for (uint8_t p=0;p<(expectedBits-1)/32;p++){
									worker->tag->pageData[(currPage+p)*4+0] = memBlock[p]>>24 & 0xff;
									worker->tag->pageData[(currPage+p)*4+1] = memBlock[p]>>16 & 0xff;
									worker->tag->pageData[(currPage+p)*4+2] = memBlock[p]>>8 & 0xff;
									worker->tag->pageData[(currPage+p)*4+3] = memBlock[p] & 0xff;
		...
		if (readSucces && backupCMD->CMDcount>=7){ 	
			readSucces=false;
			backupCMD->CMDcount=0;//required since otherwise the 'relaunch' of cmd is triggered straightaway

			//update protocolData
			size_t data_size = protocol_dict_get_data_size(worker->dict, LFRFIDProtocolHitag1);
			lfrfid_hitag_worker_combine_tag_data(worker->tag);
			protocol_dict_set_data(worker->dict, LFRFIDProtocolHitag1, worker->tag->tagData, data_size);
			
			if (currPage==0){	//read config page next via SELECT cmd
				FURI_LOG_D(TAG, "Hitag1, [%02X %02X %02X %02X] found", worker->tag->pageData[0], worker->tag->pageData[1], worker->tag->pageData[2], worker->tag->pageData[3]);
				...
				lfrfid_hitag_worker_process_config_page(worker->tag);
				FURI_LOG_D(TAG, "Hitag1, [%02X %02X %02X %02X] config page read", worker->tag->pageData[0], worker->tag->pageData[1], worker->tag->pageData[2], worker->tag->pageData[3]);
				//find next public readible block
				for (currPage=4;currPage<=HITAG_PAGES-4;currPage+=4){
					if (worker->tag->pagePublic[currPage] && (worker->tag->pageRW[currPage] & READ) ){	//public readible page/block found
						expectedBits=129;
						break;
					} else {
						FURI_LOG_D(TAG, "Hitag1, [%02X %02X %02X %02X] block %u not public/readible", worker->tag->pageData[0], worker->tag->pageData[1], worker->tag->pageData[2], worker->tag->pageData[3], currPage/4);
					}
				}
			}
			else if (currPage<HITAG_PAGES-4){ //scan for next public readible page
				FURI_LOG_D(TAG, "Hitag1, [%02X %02X %02X %02X] block %u read", worker->tag->pageData[0], worker->tag->pageData[1], worker->tag->pageData[2], worker->tag->pageData[3], currPage/4);
				for (currPage+=4;currPage<=HITAG_PAGES-4;currPage+=4){
					if (worker->tag->pagePublic[currPage] && (worker->tag->pageRW[currPage] & READ) ){	//public readible page/block found
						expectedBits=129;
						break;
			...
			else if (currPage==HITAG_PAGES-4){ //all public readible pages read: signal view & break out of while loop (stops the worker)
				FURI_LOG_D(TAG, "Hitag1, [%02X %02X %02X %02X] block %u read", worker->tag->pageData[0], worker->tag->pageData[1], worker->tag->pageData[2], worker->tag->pageData[3], currPage/4);
			...
			if (expectedBits>0){ //inject CMD
				...
				uint32_t value[] = {128,		0b00000,		SN0, 		SN1,		SN2,		SN3,		crc,		1,			213,		(uint32_t)(1.05*33*HITAG_BITPERIODS_MC)};	//always take 5% reading time buffer 
				uint8_t type[]  = {HITAG_ON,	HITAG_SELECT,	HITAG_BYTE,	HITAG_BYTE,	HITAG_BYTE,	HITAG_BYTE,	HITAG_CRC,	HITAG_STOP,	HITAG_ON,	HITAG_ON};
				uint16_t len = 10;
				uint16_t cnt = 1;
				...
				if (currPage < 4*4){	//add READPAGE cmd to array
					...
					uint32_t value2[] = {128,		0b1100,		currPage, 		crc,		1,			213,	(uint32_t)(1.05*33*HITAG_BITPERIODS_MC)};	//always take 5% reading time buffer 
					uint8_t type2[]  = {HITAG_ON,	HITAG_CMD,	HITAG_BYTE,	HITAG_CRC,	HITAG_STOP,	HITAG_ON,	HITAG_ON};
					uint16_t len2 = 7;
					uint16_t cnt2 = 5;
					...
					lfrfid_hitag_worker_calc_crc(&crc, currPage, 8);
					uint32_t value2[] = {128,		0b1101,		currPage, 		crc,		1,			213,	(uint32_t)(1.05*129*HITAG_BITPERIODS_MC)};	//always take 5% reading time buffer 
					uint8_t type2[]  = {HITAG_ON,	HITAG_CMD,	HITAG_BYTE,	HITAG_CRC,	HITAG_STOP,	HITAG_ON,	HITAG_ON};
					uint16_t len2 = 7;
					uint16_t cnt2 = 5;
					for (size_t i = 0;i<cnt2*len2;i++){
