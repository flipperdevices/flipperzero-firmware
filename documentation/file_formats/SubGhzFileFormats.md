...
- **Data_RAW** is an encoded sequence of durations, where each bit in the sequence encodes one TE interval: 1 - high level (there is a carrier), 0 � low (no carrier).
    ...
    When sending uploads, `Bit_RAW` and `Data_RAW` form a repeating block. �everal such blocks are necessary if you want to send different sequences sequentially. However, usually, there will be only one block.
