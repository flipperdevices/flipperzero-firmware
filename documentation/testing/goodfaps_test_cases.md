...
3. Save the card
   **Expected result**
     1. Card succesfuly saved

4. Click Elite Dictionary Attack
   **Expected result**
     1. The attack will begin
...
2. Use the second Flipper to simulate any weather station at different frequencies, and click Read on the first
   **Expected result**
     1. Reading the weather station succeeded

3. Press Config and enable Hopping
   **Expected result**
     1. Hopping works and stations are read on different frequencies

4. Press Config and lock screen
   **Expected result**

     1. Screen lock working

5. Exit the reading menu and go to the About menu
   **Expected result**
     1. Information in the About menu is displayed
...
1. Connect the flipper to a companion or web platform
   **Expected result**
     1. Flipper connected successfully

2. Install any number of applications
   **Expected result**
     1. The applications were installed and appeared in the list with a normal icon and the same name as in the
        directory

3. Launch the application
   **Expected result**
     1. The application has launched and is working normally

4. Quit the application
   **Expected result**
     1. The application closed and the flipper continued to work normally

5. The application closed and the flipper continued to work normally
   **Expected result**
     1. The applications have been successfully updated and are working

6. Uninstall multiple apps using companion, web platform and file browser
   **Expected result**
     1. Uninstall multiple applications using the companion, the web platform and through the file browser of the
        flipper itself
...
3. Launch Flash AVR and flash the chip
   **Expected result**
     ...
     3. ÐÑÐ¾ÑÐ¸Ð²ÐºÐ° Ð·Ð°Ð¿Ð¸ÑÐ°Ð»Ð°ÑÑ Ð²ÐµÑÐ½Ð¾

4. Launch ISP programmer
   **Expected result**
     1. ISP Programmer correctly identified the chip
...
2. Insert the HDMI cable already connected to the monitor into the module
   **Expected result**
     1. The image from the flipper began to duplicate on the monitor

3. Download and run the game "Air Arkanoid"
   **Expected result**
     1. Accelerometer control works
...
1. Go to Apps > Tools > Clock
   **Expected result**
     1. The clock has activated full screen and is running. The clock format is determined in the flipper settings

2. ÐÐ°Ð¹ÑÐ¸ Ð² Apps > Tools > NFC/Rfid Detector
3. Click on About
   **Expected result**
     1. A tab with information about the application has opened

4. Click Detect Field Type
   **Expected result**
     1. The application work area has opened, the screen is blank

5. Bring the flipper to any reader or even two nearby ones
   **Expected result**
     1. The flipper recognizes both NFC and RFID; if two readers are nearby, both types of cards will be displayed
...
2. Check on different outputs and frequencies
   **Expected result**
     ...
     1. The PWM signal is generated with the specified parameters on the specified pin

3. Run Clock Generator and Test at different frequencies and dividers
   **Expected result**
     1. The clock signal is generated with the specified parameters on the specified pin
...
2. Click Check Magic Tag and attach the card of the required type
   **Expected result**
     1. The card was considered

3. Click Write Gen1a and select the desired card from the list of saved ones
   **Expected result**
     1. A message appeared about the riskiness of the operation, after confirmation the information was recorded on
        the card

4. Press "Wipe"
   **Expected result**
     1. Information from the card has been deleted
...
3. Launch the application
   **Expected result**
     1. If the device is connected to a monitor, screen mirroring will stop.
