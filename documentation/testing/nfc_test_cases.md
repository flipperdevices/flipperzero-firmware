...
1. Auth as. Both types work identically. Bring cards of both types, and wrong type card
   **Expected result**
     1. If the card type is correct, the card is successfully counted.

     3. If the card type is incorrect, a message about invalid card type will appear and we will be returned to the
        reading screen.

2. Save scanned cards
   **Expected result**
     1. Cards saved, after one and a half seconds the scene will change to the "saved" menu
...
3. Enter password manually, by attaching a card of the correct type and a card of the wrong type
   **Expected result**
     ...
     4. When you apply a card of the wrong type, the scene will change to a warning window about the wrong card
        type
...
2. Bring the flipper to the active NFC Reader (for example, another flipper)
   **Expected result**
     ...
     1. Nonces were considered with a light-vibration signal

3. Press OK
   **Expected result**
     1. A screen with a QR code appears, which is correctly read by the camera

## 3. NFC Emulation

Steps:
1. Launch NFC â Save card â Emulate
...
1. Press "Read Specific card type", and read the key types of cards
   **Expected result**
     1. The cards were counted, the reading is done in a standard way

2. Press Mifare Classic Keys, and add some key
   **Expected result**
     1. Keys have been added, a "List" button has appeared on the main screen

3. Click the "list" button and delete any key
   **Expected result**
     1. The deleted key disappeared from the list, if the last key is deleted, the "list" button disappears from the main
       menu

4. Press "Unlock SLIX", and Enter Password
   **Expected result**
     1. If the password is correct, by placing the card it will be unlocked.

5. View info, simulate and save map
   **Expected result**
     1. The info is correct, the emulation works, the card is saved

6. Apply the Tommy Box toy card, repeat steps 4 and 5 without entering the password
   **Expected result**
     1. **Expected result**s of steps 4 and 5
