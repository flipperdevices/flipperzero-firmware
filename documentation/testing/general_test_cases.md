...
1. Go to Settings > BT > BT ON, and return to the main screen
   **Expected result**
     1. BT icon appears after switching on

2. Restart device
   **Expected result**
     1. After reboot BT is on
...
5. Open the companion on another phone and repeat the pairing
   **Expected result**
     1. The pairing is working again

6. Close the application and connect the first companion
   **Expected result**
     1. The application has connected
...
1. Select PIN Setup
    **Expected result**
      1. Instructions for setting a password appeared
...
3. Lock your device and press any button
    **Expected result**
      1. When pressed, a password entry window appears

4. Enter the wrong password
    **Expected result**
      1. If the password is incorrect, the password repeat window and the red indicator is on
...
6. Enter the correct password
    **Expected result**
      1. Flipper is unlocked with the correct password

7. Go to Settings > Desktop and change password
    **Expected result**

      1. The password has changed and is correct

8. Go to Settings > Desktop and remove password
    **Expected result**
      1. The password has been deleted

9. Enable automatic blocking of a device with an added pin
    **Expected result**
      1. Flipper locked automatically

10. Unlock Device
    **Expected result**
      1. Flipper unlocked with the correct pin

11. Reboot device and
    Unlock
    **Expected result**
      1. Flipper was locked after reboot and unlocked successfully

## 4. Settings. Storage

Steps:
1. Settings â Storage â About internal storage
   **Expected result**
     1. Displays device name, file system type
     2. Total storage capacity and available capacity in kilobytes

2. Settings â Storage â About SD Card
   **Expected result**
...
5. Format SD Card (It's better to use another SD)
   **Expected result**
     1. Formatting was successful

6. Factory reset. Factory reset, which resets everything, including all installed applications and region (Its better to
   use another Flipper)
   **Expected result**
     1. The reset occurred, and all the specified settings were set to factory
...
1. Insert a card with a large number of files on the SD card and pre-recorded keys and cards
   **Expected result**
     1. The map was successfully identified.

2. On the main screen, press DOWN and go to different sections (Favorites, iButton, NFC, SubOne, Rfid, Infrared,
   Browser)
   **Expected result**
     1. Each section has stored keys/cards

3. Run keys through each section
   **Expected result**
     1. The keys have been launched successfully.

4. Rename multiple keys
   **Expected result**
     1. Keys successfully renamed

5. Delete multiple keys
   **Expected result**
     1. Keys successfully deleted

## 6. "Dummy mode"

Steps:
1. It is recommended to install in the settings, applications in quick access for dummy mode, for buttons LEFT,
   DOWN, RIGHT and their clamping
2. On the main screen press UP > Dummy mode
   **Expected result**
     1. Upon exiting to the desktop, the gamepad icon appeared. When you press the central button (OK), the
        Passport opens

3. Press, LEFT, BOTTOM, RIGHT, and also press and hold them
   **Expected result**
     1. Exactly those applications assigned to a specific button open and hold down the button

4. On the main screen press UP > Default mode
   **Expected result**
     1. Dummy mode is disabled, the gamepad icon has disappeared, when you click OK, the application menu opens
...
1. Rollback Flipper firmware to 0.64.5
  ...
  2.1. Update on latest stable version of companion (OS) was successful
...
1. Reset region on Flipper by deleting region data
   ...
   4. Run update on target system
   4.1. Region correlates to the internet location
   5. Verify bands in the app
   5.1. bands correctly correspond to test region
...
2. Reboot â Flipper OS
   **Expected result**
     1. A dialog box appears
     2. Flipper has rebooted into the system

3. Reboot â Firware upgrade
   **Expected result**
     1. A dialog box appears
...
4. Power Off
   **Expected result**
     ...
     3. When the cable is connected, after clicking âDisconnectâ, the âYou can now disconnect the cableâ screen will
        appear. Once disconnected, the device will turn off
...
1. Hand orient change to Lefty
    **Expected result**
      1. The image on the screen will turn upside down, and the control buttons will react in a mirror way

2. Units change to imperial
    **Expected result**
      1. Units of measurement, wherever they are, will be displayed in imperial format

3. Change Time Format to 12h
    **Expected result**
      1. The time will change to AM\PM format

4. Date format change to any other
    **Expected result**
      1. The position of the date elements will change

5. Change Log Level
    **Expected result**
      1. Displaying logs in the CLI\Putty interface

6. Enable/disable debug
    **Expected result**
      1. The ability to debug the device will be enabled, for example in VS Code

7. Enable Trace
    **Expected result**
      1. The device will start to slow down, and when you exit or enter any menu, you will receive a detailed log
         about this

8. Change Sleep Method to Legacy
    **Expected result**
      1. The device will no longer enter energy saving mode, and in the battery menu we will see the energy
         consumption permanently

9. Change file naming to Detailed
    **Expected result**
      1. When creating a key without specifying your own name, the name has its own format from
         âUntolded_door_â to âKey_type_dd_mm_yyyyâ

10. Change log device from USART
    **Expected result**
      1. After the change, the Game module will stop working and being recognized

## 27. Settings. Bluetooth

Steps:
1. Settings â Bluetooth â on
   **Expected result**
     1. Bt icon appeared on the main screen
     2. The device appears in the list for connection

2. Settings â Bluetooth â off
   **Expected result**
     1. The icon disappeared, the device disappeared from the search / disconnected from the companion
...
Ð¡hecking whether the flipper name is displayed correctly in different scenarios
Steps:
1. Press the DOWN button and make sure the name is there
  2. Go to Settings > Passport
  2.1. The name is on the menu
  2.2. Attachment [39]
  3. On Linux, run dmesg -w. Connect the device via USB, make sure that the name is in the device name and in the
  Serial number ls -l /dev/serial/by-id/
  3.1. The name is in USB Serial Number
  4. On Windows, go to settings>bluetooth and devices>devices
  4.1. The device was displayed with the desired name among other devices
  4.2. Attachment [40]
  5. Check on MacBook that the device name is in the list of devices
  5.1. The name is on the MacBook
  6. Check the name in FlipperLAB
  6.1. In the web interface the name is the same as in the previous steps
  6.2. Attachment [67]
...
2. Set Quick Access Apps
   **Expected result**
     1. A dialog box appears with a choice of settings for standard mode and dummy mode

3. Install applications for each button in both modes and check launch
   **Expected result**
     1. Each button launches exactly the application to which it is assigned

4. Remove one or more applications that are set to quick access and try to launch
   **Expected result**
     1. Startup error screen appears
...
4. In the Backlight time setting, check the screen operating time
   **Expected result**
     1. Screen turns off after 1, 5, 15, 30, 60, 120 seconds
...
7. In the Vibro setting, change the value to OFF, and back to ON
   **Expected result**
     1. When changing the setting from OFF - ON, vibration occurs
...
4. Open companion and exit the BT remote application
   **Expected result**
     1. Flipper reconnected with his companion

5. USB Remote. Connect the flipper to your PC or phone using a cable and open the application, selecting the
   desired mode
   **Expected result**
     1. It just works in mouse or keyboard mode
...
2. Lock the device using the "lock" item
   **Expected result**
     1. The device is locked with animation
...
4. MUTE. Press Mute button
   **Expected result**
     1. A crossed out note icon appeared on the main screen

5. UNMUTE
   **Expected result**
     1. The icon has disappeared
...
2. Follow the script, going into each submenu
   **Expected result**
     1. Each menu will have exactly what is stated in the title.

3. Run a delay script
   **Expected result**
     1. The count to 3 occurred, the script completed successfully
