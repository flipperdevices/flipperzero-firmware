...
1. Create a folder for your new app, name it as you want
2. In this folder, open Terminal and run: `ufbt create APPID=app_name`. uFBT will generate the file structure required for the app.
...
In a  `test_app` app, the file structure will looks as follows:
...
> - `FURI_LOG_I` — A function that logs a message to the Flipper console. You can see the logs by opening the console (`ufbt cli`), then running the `log` command.
...
2. In qFlipper, go to **File Manager > SD Card > apps**, then simply drag and drop your built app into the folder.
3. Run your app. On your Flipper Zero, go to **Applications** → **User** and select your app.
...
If you used the log output method (`FURI_LOG_I`), you can see the logs by opening the console (`ufbt cli`), then running the `log` command.
