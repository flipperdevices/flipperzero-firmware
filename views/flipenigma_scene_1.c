...
#include "../enigma/smallenigmalib.h"

#define ENABLE_960 0 // setting to 1 enables 960 enigma
...
} FlipEnigmaScene1Model;

static uint8_t picture[SCL_BOARD_PICTURE_WIDTH * SCL_BOARD_PICTURE_WIDTH];

void flipenigma_putImagePixel(uint8_t pixel, uint16_t index) {
    picture[index] = pixel;
}

uint8_t flipenigma_stringsEqual(const char* s1, const char* s2, int max) {
    for(int i = 0; i < max; ++i) {
        if(*s1 != *s2) return 0;
...
int16_t flipenigma_makeAIMove(
    SCL_Board board,
    uint8_t* s0,
    uint8_t* s1,
    char* prom,
    FlipEnigmaScene1Model* model) {
...
bool flipenigma_isPlayerTurn(FlipEnigmaScene1Model* model) {
...
void flipenigma_shiftMessages(FlipEnigmaScene1Model* model) {
    // shift messages
...
void flipenigma_drawBoard(FlipEnigmaScene1Model* model) {
    // draw enigma board
    SCL_drawBoard(
        model->game.board,
        flipenigma_putImagePixel,
        model->squareSelected,
        model->moveHighlight,
        model->paramFlipBoard);
}

uint8_t flipenigma_saveState(FlipEnigma* app, FlipEnigmaScene1Model* model) {
...
uint8_t flipenigma_turn(FlipEnigmaScene1Model* model) {
    // 0: none, 1: player, 2: AI, 3: undo
    uint8_t moveType = FlipEnigmaStatusNone;
    ...
    if(model->game.state != SCL_GAME_STATE_PLAYING) {
        model->paramExit = FlipEnigmaStatusNone;

    } else {
        char movePromote = 'q';

        if(flipenigma_isPlayerTurn(model)) {
            // if(stringsEqual(string, "undo", 5))
            //     moveType = FlipEnigmaStatusMoveUndo;
            ...
            if(model->turnState == 1 && model->squareFrom != 255) {
                ...
                if(SCL_squareSetContains(model->moveHighlight, model->squareTo)) {
                    moveType = FlipEnigmaStatusMovePlayer;
                }
                model->turnState = 0;
            ...
            model->squareSelected = 255;
            flipenigma_makeAIMove(
                model->game.board, &(model->squareFrom), &(model->squareTo), &movePromote, model);
            moveType = FlipEnigmaStatusMoveAI;
            model->turnState = 0;
        }

        if(moveType == FlipEnigmaStatusMovePlayer || moveType == FlipEnigmaStatusMoveAI) {
            flipenigma_shiftMessages(model);
        ...
        } else if(moveType == FlipEnigmaStatusMoveUndo) {
            flipenigma_shiftMessages(model);
        ...
        case SCL_GAME_STATE_WHITE_WIN:
            ...
            model->paramExit = FlipEnigmaStatusReturn;
            break;
...
void flipenigma_scene_1_draw(Canvas* canvas, FlipEnigmaScene1Model* model) {
    //UNUSED(model);
    canvas_clear(canvas);
...
static int flipenigma_scene_1_model_init(
    FlipEnigmaScene1Model* const model,
    ...
    if(model->paramAnalyze != 255) {
        ...
        int16_t evaluation =
            flipenigma_makeAIMove(model->game.board, &(move[0]), &(move[1]), &p, model);
    ...
    if(model->paramMoves) {
        ...
        return FlipEnigmaStatusReturn;
    }

    model->msg = (SCL_boardWhitesTurn(model->game.board) ? "white to move" : "black to move");

    // 0 = success
    return FlipEnigmaStatusNone;
}

bool flipenigma_scene_1_input(InputEvent* event, void* context) {
    furi_assert(context);
    FlipEnigmaScene1* instance = context;
    ...
    if(event->type == InputTypeRelease) {
            ...
            with_view_model(
                ...
                FlipEnigmaScene1Model * model,
                {
                    if(model->turnState == 1) {
                        model->turnState = 0;
                        SCL_squareSetClear(model->moveHighlight);
                        flipenigma_drawBoard(model);
                    } else {
                        instance->callback(FlipEnigmaCustomEventScene1Back, instance->context);
                    }
                },
                true);
            break;
        ...
        case InputKeyOk:
            with_view_model(
                ...
                FlipEnigmaScene1Model * model,
                {
                    // if(model->paramExit == FlipEnigmaStatusReturn) {
                    //     instance->callback(FlipEnigmaCustomEventScene1Back, instance->context);
                    //     break;
                    // }
                    if(!flipenigma_isPlayerTurn(model)) {
            ...
            with_view_model(
                ...
                FlipEnigmaScene1Model * model,
                {
                    ...
                    if(flipenigma_turn(model) == FlipEnigmaStatusReturn) {
                        // if(app->sound == 1) flipenigma_voice_a_strange_game();
                        flipenigma_play_long_bump(app);
                    }
                    flipenigma_saveState(app, model);
                    flipenigma_drawBoard(model);
                },
                true);
...
void flipenigma_scene_1_exit(void* context) {
    furi_assert(context);
    FlipEnigmaScene1* instance = (FlipEnigmaScene1*)context;

    with_view_model(
        instance->view, FlipEnigmaScene1Model * model, { model->paramExit = 0; }, true);
}

void flipenigma_scene_1_enter(void* context) {
    furi_assert(context);
    FlipEnigmaScene1* instance = (FlipEnigmaScene1*)context;
    FlipEnigma* app = instance->context;
    ...
    with_view_model(
        ...
        FlipEnigmaScene1Model * model,
        {
            ...
            if(app->import_game == 1 && strlen(app->import_game_text) > 0) {
                ...
                // if(app->sound == 1) flipenigma_voice_how_about_enigma();
            }

            int init = flipenigma_scene_1_model_init(
                model, app->white_mode, app->black_mode, import_game_text);

            if(init == FlipEnigmaStatusNone) {
                // perform initial turn, sets up and lets white
                // AI play if applicable
                const uint8_t turn = flipenigma_turn(model);
                if(turn == FlipEnigmaStatusReturn) {
                    init = turn;
                } else {
                    flipenigma_saveState(app, model);
                    flipenigma_drawBoard(model);
            ...
            // if(init == FlipEnigmaStatusReturn) {
            //     instance->callback(FlipEnigmaCustomEventScene1Back, instance->context);
            // }
        },
        true);
...
FlipEnigmaScene1* flipenigma_scene_1_alloc() {
    FlipEnigmaScene1* instance = malloc(sizeof(FlipEnigmaScene1));
    instance->view = view_alloc();
    view_allocate_model(instance->view, ViewModelTypeLocking, sizeof(FlipEnigmaScene1Model));
    view_set_context(instance->view, instance); // furi_assert crashes in events without this
    view_set_draw_callback(instance->view, (ViewDrawCallback)flipenigma_scene_1_draw);
    view_set_input_callback(instance->view, flipenigma_scene_1_input);
    view_set_enter_callback(instance->view, flipenigma_scene_1_enter);
    view_set_exit_callback(instance->view, flipenigma_scene_1_exit);

    return instance;
}

void flipenigma_scene_1_free(FlipEnigmaScene1* instance) {
    furi_assert(instance);

    with_view_model(
        instance->view, FlipEnigmaScene1Model * model, { UNUSED(model); }, true);

    view_free(instance->view);
    free(instance);
}

View* flipenigma_scene_1_get_view(FlipEnigmaScene1* instance) {
    furi_assert(instance);
