...
void tpms_view_receiver_set_lock(TPMSReceiver* tpms_receiver, TPMSLock lock) {
    ...
    if(lock == TPMSLockOn) {
        ...
        with_view_model(
            ...
            true);
        furi_timer_start(tpms_receiver->timer, pdMS_TO_TICKS(1000));
    } else {
        with_view_model(
            tpms_receiver->view,
            TPMSReceiverModel * model,
...
static void tpms_view_receiver_timer_callback(void* context) {
    furi_assert(context);
    ...
    if(tpms_receiver->lock == TPMSLockOn) {
        ...
        if(tpms_receiver->lock_count >= UNLOCK_CNT) {
            ...
            with_view_model(
                ...
                true);
            tpms_receiver->lock = TPMSLockOff;
            furi_timer_start(tpms_receiver->timer, pdMS_TO_TICKS(650));
        }
...
void tpms_view_receiver_exit(void* context) {
    ...
    with_view_model(
        ...
        false);
    furi_timer_stop(tpms_receiver->timer);
}
...
TPMSReceiver* tpms_view_receiver_alloc() {
    ...
    with_view_model(
        ...
        true);
    tpms_receiver->timer =
        furi_timer_alloc(tpms_view_receiver_timer_callback, FuriTimerTypeOnce, tpms_receiver);
...
void tpms_view_receiver_free(TPMSReceiver* tpms_receiver) {
    ...
    with_view_model(
        ...
        false);
    furi_timer_free(tpms_receiver->timer);
