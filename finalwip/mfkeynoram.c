...
int recover(int odd[], int o_head, int o_tail, int oks, int even[], int e_head, int e_tail, int eks, int rem,
            int s, struct Crypto1Params *p, int ks2, int first_run) {
    int o, e, i;
    if (rem == -1) {
    ...
    if (first_run) {
        ...
        odd = extend_table_precompute(e_head, &o_tail, oks, 8, CONST_M1_1, CONST_M2_1);
        even = extend_table_precompute(e_head, &e_tail, eks, 8, CONST_M1_2, CONST_M2_2);
    ...
    while (o_tail >= o_head && e_tail >= e_head) {
        if (((odd[o_tail] ^ even[e_tail]) >> 24) == 0) {
            ...
            s = recover(odd, o_tail--, o, oks, even, e_tail--, e, eks, rem, s, p, ks2, 0);
            if (s == -1) {
                break;
            }
...
int main(int argc, char *argv[]) {
    ...
    while(fgets(buffer, bufferLength, filePointer)) {
        ...
        recover(0, 0, 0, 0, 0, 0, 0, 0, 7, 0, &p, ar0_enc ^ p64, 1);
        found_key = p.key;
        int found = 0;
