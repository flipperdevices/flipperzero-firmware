...
void Serial_prints(const char* c) {
    size_t sent = furi_stream_buffer_send(context->dmcomm_output_stream, c, strlen(c), 0);
    UNUSED(sent);
}
...
void Serial_printlns(const char* c) {
    ...
    sent = furi_stream_buffer_send(context->dmcomm_output_stream, "\n", 1, 0);
    UNUSED(sent);

    if(serialCallback != NULL) (*serialCallback)(context);
}

void Serial_println(void) {
    size_t sent = furi_stream_buffer_send(context->dmcomm_output_stream, "\n", 1, 0);
    UNUSED(sent);
...
void Serial_printi(const int c) {
    ...
    size_t sent = furi_stream_buffer_send(context->dmcomm_output_stream, str, strlen(str), 0);
    UNUSED(sent);
}
...
int Serial_read(void) {
    ...
    recieved = furi_stream_buffer_receive(context->dmcomm_input_stream, &s, 1, 1);
...
void Serial_writeb(const byte* data, int len) {
    ...
    for(int i = 0; i < len; i++) {
        ...
        sent = furi_stream_buffer_send(context->dmcomm_output_stream, str, 1, 0);
    }
    UNUSED(sent);
}