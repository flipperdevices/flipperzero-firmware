...
static void input_callback(InputEvent* input_event, void* ctx) {
    ...
    furi_message_queue_put(event_queue,
                           input_event,
                           FuriWaitForever);
}
...
int32_t si4713fmtuner_app(void* p) {
    ...
    ctx->event_queue = furi_message_queue_alloc(8,
                                                sizeof(InputEvent));
    if (!ctx->event_queue) {
    ...
    view_port_draw_callback_set(ctx->viewport,
                                canvas_render_callback,
                                ctx);
    view_port_input_callback_set(ctx->viewport,
                                 input_callback,
                                 ctx->event_queue);
...
//
//        }
        if (furi_message_queue_get(ctx->event_queue,
                                  &event,
                                  50) != FuriStatusOk) continue;
        ...
        case InputKeyOk:
            if (event.type == InputTypeShort) {
                ctx->state.working =
                    ctx->state.working == DeviceOff ? DeviceOn : DeviceOff;
                si4713_set_tx_power(ctx->device,
                                    ctx->state.working == DeviceOn ? ctx->state.tx_power : 0,
                                    0);
                if (ctx->state.working != DeviceOff)
                    si4713_tune_fm(ctx->device, ctx->state.freq);
            }
            break;
        case InputKeyBack:
