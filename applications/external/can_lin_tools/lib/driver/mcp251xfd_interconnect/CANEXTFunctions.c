...
{
  //--- Controller clocks ---
  .XtalFreq       = MCP251XFD_EXT2_CLK,        // CLKIN is an crystal
...
MCP251XFD_FIFO MCP2517FD_Ext2_FIFOlist[MCP2517FD_EXT2_FIFO_COUNT] =
{
 { .Name = MCP251XFD_TEF,   .Size = MCP251XFD_FIFO_10_MESSAGE_DEEP,                                                                                                                                                               .ControlFlags = MCP251XFD_FIFO_ADD_TIMESTAMP_ON_OBJ,  .InterruptFlags = MCP251XFD_FIFO_OVERFLOW_INT + MCP251XFD_FIFO_EVENT_FIFO_NOT_EMPTY_INT,                .RAMInfos = &Ext2_TEF_RAMInfos, },
 { .Name = MCP251XFD_TXQ,   .Size = MCP251XFD_FIFO_4_MESSAGE_DEEP,  .Payload = MCP251XFD_PAYLOAD_64BYTE,                                       .Attempts = MCP251XFD_THREE_ATTEMPTS, .Priority = MCP251XFD_MESSAGE_TX_PRIORITY16, .ControlFlags = MCP251XFD_FIFO_NO_RTR_RESPONSE,       .InterruptFlags = MCP251XFD_FIFO_TX_ATTEMPTS_EXHAUSTED_INT + MCP251XFD_FIFO_TRANSMIT_FIFO_NOT_FULL_INT, .RAMInfos = &Ext2_TXQ_RAMInfos, },
...
// //=============================================================================
// void MCP251XFD_CLKO_Handler(uint32_t id, uint32_t mask)
// {
//   Ext2CLKOCounter++;
//   nop();
// }
...
{
  MCP251XFD_Ext2.UserDriverData = &MCP2517FD_Ext2_Config; // This is to allow the MCP251XFD_Ext2_IntPinInit(), MCP251XFD_Ext2_Int0Gpio0PinInit() and MCP251XFD_Ext2_Int1Gpio1PinInit() to get the pins configurations in this demo

  //--- Initialize Int pins or GPIOs ---
  MCP251XFD_IntPinInit(CANEXT2);
  MCP251XFD_Int0Gpio0PinInit(CANEXT2);
  MCP251XFD_Int1Gpio1PinInit(CANEXT2);
...
eERRORRESULT TransmitMessageFromEXT2_No_IRQ(uint32_t messageID, uint32_t* messageSEQ, setMCP251XFD_MessageCtrlFlags controlFlags,  eMCP251XFD_DataLength dlc, uint8_t* payloadData, bool flush) {
    ...
    ErrorExt2 = MCP251XFD_GetTransmitPendingInterruptStatusOfAllFIFO(
        CANEXT2, &InterruptOnFIFO); // Get all FIFO status
    ...
    for(eMCP251XFD_FIFO zFIFO = 0; zFIFO < MCP251XFD_TX_FIFO_MAX;
        ...
        if((InterruptOnFIFO & (1 << zFIFO)) > 0) // If an Interrupt is flagged
        {
            ErrorExt2 = MCP251XFD_GetFIFOStatus(
                CANEXT2, zFIFO, &FIFOstatus); // Get the status of the flagged FIFO
            if(ErrorExt2 != ERR_OK) return ErrorExt2;
            if((FIFOstatus & MCP251XFD_TX_FIFO_NOT_FULL) > 0) // Check FIFO not empty
            {
                ...
                ErrorExt2 = MCP251XFD_TransmitMessageToFIFO(CANEXT2, &TansmitMessage, zFIFO, flush); // Send message and flush
            }
        }
    return ErrorExt2;
}
...
eERRORRESULT ReceiveMessageFromEXT2(MCP251XFD *pComp, bool* receive_event, MCP251XFD_CANMessage *msg, eMCP251XFD_PayloadSize payload_size, uint32_t *msg_time_stamp)
{
  ...
  {
    ErrorExt2 = MCP251XFD_GetCurrentReceiveFIFONameAndStatusInterrupt(pComp, &FIFOname, &FIFOstatus);
  ...
  eMCP251XFD_FIFOstatus FIFOstatus = 0;
  setMCP251XFD_InterruptOnFIFO InterruptOnFIFO = 0;
  ErrorExt2 = MCP251XFD_GetReceivePendingInterruptStatusOfAllFIFO(pComp, &InterruptOnFIFO); // Get all FIFO status
  if (ErrorExt2 != ERR_OK) return ErrorExt2;
  for (eMCP251XFD_FIFO FIFOname = 1; FIFOname < MCP251XFD_FIFO_MAX; FIFOname++)              // For each receive FIFO but not TEF, TXQ
    ...
    {
      ErrorExt2 = MCP251XFD_GetFIFOStatus(pComp, FIFOname, &FIFOstatus);                   // Get the status of the flagged FIFO
      if (ErrorExt2 != ERR_OK) return ErrorExt2;
...

      {
        ...
        // }
        ErrorExt2 = MCP251XFD_ReceiveMessageFromFIFO(pComp, msg, payload_size, msg_time_stamp, FIFOname);
        *receive_event =true;
      }
    }
  //*receive_event = false;
...
void CheckDeviceINTOnEXT2(void)
{
    ...
    switch (InterruptCode)
    {
      ...
      case MCP251XFD_RECEIVE_FIFO_OVF:        // Receive FIFO Overflow Interrupt
        ...
        for (eMCP251XFD_FIFO zFIFO = 1; zFIFO < MCP251XFD_RX_FIFO_MAX; zFIFO++)      // For each receive FIFO but not the TEF
          ...
          {
            FURI_LOG_E(TAG,"Ext2: Receive FIFO%u Overflow", (unsigned int)zFIFO);          // Display an error for this FIFO
      ...
      case MCP251XFD_TRANSMIT_ATTEMPT:        // Transmit Attempt Interrupt
        ...
        for (eMCP251XFD_FIFO zFIFO = 0; zFIFO < MCP251XFD_TX_FIFO_MAX; zFIFO++)      // For each transmit FIFO
          ...
          {
            FURI_LOG_E(TAG,"Ext2: Transmit FIFO%u Attempts Exhaust", (unsigned int)zFIFO); // Display an error for this FIFO
      ...
      case MCP251XFD_RXTX_MAB_OVF_UVF:        // RX MAB Overflow (RX: message received before previous message was saved to memory)
        FURI_LOG_E(TAG,"Ext2: RX MAB Overflow (RX: message received before previous message was saved to memory)");
        MCP251XFD_ClearInterruptEvents(CANEXT2, MCP251XFD_INT_SYSTEM_ERROR_EVENT);
        break;
      ...
      default:
        ...
        if ((Events & MCP251XFD_INT_SPI_CRC_EVENT) > 0) // SPI CRC event
        {
          ...
          else FURI_LOG_E(TAG,"Ext2: Number of Bytes mismatch during 'SPI with CRC' command occurred");
        ...
        if ((Events & MCP251XFD_INT_RAM_ECC_EVENT) > 0) // ECC event
        {
          ...
          else FURI_LOG_E(TAG,"Ext2: ECC Double Error was detected");
  ...
  if (TransmitError)
    FURI_LOG_W(TAG,"TransmitError = %d", InterruptCode);
