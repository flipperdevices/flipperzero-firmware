...
{
    canvas_draw_box(canvas, obstacle->positionX, obstacle->positionY, obstacle->largeur, obstacle->longueur);
}
...
bool joueurToucheObstacle(Obstacle* obstacle, int xJoueur, int yJoueur)
{
    ...
    bool toucheX = (xJoueur < (obstacle->positionX + obstacle->largeur)) && ((xJoueur + 4) > obstacle->positionX);
    bool toucheY = (yJoueur < (obstacle->positionY + obstacle->longueur)) && ((yJoueur + 4) > obstacle->positionY);
...
int32_t crossy_road() {
    ...
    FuriTimer* TimerDeplacementObstacle = furi_timer_alloc(CallbackDeplacementObstacle, FuriTimerTypePeriodic, &obstacleBouge);
