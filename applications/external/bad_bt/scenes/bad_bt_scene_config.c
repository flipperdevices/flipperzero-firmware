...
void bad_bt_scene_config_on_enter(void* context) {
    ...
    view_dispatcher_switch_to_view(bad_bt->view_dispatcher, BadBtAppViewVarItemList);
}
...
bool bad_bt_scene_config_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case VarItemListIndexRandomizeBtMac:
            furi_hal_random_fill_buf(bad_bt->config.bt_mac, BAD_BT_MAC_LEN);
            bad_bt_config_refresh(bad_bt);
            break;
        default:
            break;
