...
GravityItem settings[NUM_SETTINGS_ITEMS] = {
    {"Synchronise",
...
static void displaySubmenu(GravityApp* app, GravityItem* item) {
    ...
    if(!strcmp(item->item_string, "Purge Strategy")) {
        ...
        newScene = GravitySceneSettingsMac;
    }
    if(newScene < 0) {
        return;
    }
...
static void gravity_scene_settings_var_list_enter_callback(void* context, uint32_t index) {
    furi_assert(context);
...
static void gravity_scene_settings_var_list_change_callback(VariableItem* item) {
    furi_assert(item);
    ...
    const GravityItem* menu_item = &settings[app->selected_menu_items[GRAVITY_MENU_SETTINGS]];
...
void gravity_scene_settings_on_enter(void* context) {
    GravityApp* app = context;
    VariableItemList* var_item_list = app->settings_menu_list;
    VariableItem* item;

    variable_item_list_set_enter_callback(
        var_item_list, gravity_scene_settings_var_list_enter_callback, app);
    ...
    for(int i = 0; i < NUM_SETTINGS_ITEMS; ++i) {
        item = variable_item_list_add(
            ...
            gravity_scene_settings_var_list_change_callback,
            app);
...
bool gravity_scene_settings_on_event(void* context, SceneManagerEvent event) {
    UNUSED(context);
    GravityApp* app = context;
    bool consumed = false;
...
/* Clean up on exit */
void gravity_scene_settings_on_exit(void* context) {
    GravityApp* app = context;
