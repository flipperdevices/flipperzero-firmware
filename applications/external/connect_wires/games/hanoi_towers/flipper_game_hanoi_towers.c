...
static void draw_game(Canvas* canvas, GameState* gs) {
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    ...
    canvas_draw_line(canvas, towerX-platformW, SCREEN_HEIGHT-1, towerX+platformW-1, SCREEN_HEIGHT-1);
    canvas_set_color(canvas, ColorWhite);
...
void draw_menu(Canvas* canvas, const MenuEntry* menu, uint8_t selectedIndex) {
  ...
  for (uint8_t i = 0; i < nitems; ++i) {
    canvas_set_color(canvas, ColorBlack);
    if (i == selectedIndex) {
      canvas_draw_box(canvas, SCREEN_WIDTH/2-max_width/2, SCREEN_HEIGHT/2-item_h*nitems/2+i*item_h, max_width, item_h);
    }
    canvas_set_color(canvas, i == selectedIndex ? ColorWhite : ColorBlack);
    canvas_draw_str_aligned(canvas, SCREEN_WIDTH/2, SCREEN_HEIGHT/2-item_h*nitems/2+i*item_h+item_h/2, AlignCenter, AlignCenter, menu[i].text);
  }
  canvas_set_color(canvas, ColorBlack);
  canvas_draw_rframe(canvas, SCREEN_WIDTH/2-max_width/2-DLT_FRAME, SCREEN_HEIGHT/2-item_h*nitems/2-DLT_FRAME, max_width+DLT_FRAME*2, item_h*nitems+DLT_FRAME*2, 2);
}
...
void draw_about(Canvas* canvas) {
  ...
  for (uint8_t i = 0; i < nitems; ++i) {
    ...
    canvas_draw_str_aligned(canvas, SCREEN_WIDTH/2-max_width/2, SCREEN_HEIGHT/2-item_h*nitems/2+i*item_h+item_h/2, AlignLeft, AlignCenter, AboutStrings[i]);
  }
}
...
void draw_message_box(Canvas* canvas, const MessageBoxLine* lines) {
  ...
  canvas_set_color(canvas, ColorWhite);
  canvas_draw_box(canvas, SCREEN_WIDTH/2 - maxW/2 - paddingH, SCREEN_HEIGHT/2 - totalH/2 - paddingV, maxW + paddingH*2, totalH + paddingV*2);
  canvas_set_color(canvas, ColorBlack);
  canvas_draw_rframe(canvas, SCREEN_WIDTH/2 - maxW/2 - paddingH, SCREEN_HEIGHT/2 - totalH/2 - paddingV, maxW + paddingH*2, totalH + paddingV*2, 2);
  uint8_t currY = SCREEN_HEIGHT/2 - totalH/2 + 1;
  
  for (uint8_t i = 0; i < nlines; ++i) {
    canvas_set_font(canvas, lines[i].bold ? FontPrimary : FontSecondary);
    uint8_t lineH = canvas_current_font_height(canvas);
    canvas_draw_str_aligned(canvas, SCREEN_WIDTH/2, currY, AlignCenter, AlignTop, lines[i].text);
    currY += lineH;
  }
}
...
void draw_finishing(Canvas* canvas, GameState* gs) {
  ...
  const char* line2pattern = "Total moves: %d";
  const char* line3src = "\x49\x69\x43\xc\x2\x1d\x1c\x9\x11\x11\x1\x44\x68\x29\xf\x1\x6\x49\x74\x3b\x18\x12\x17\x1";
  const char* line4src = "\x61\xf\xa\x44\x41\xd\x0\x4c\x69\x69\x47\x8\x1b\x54\x57\x16\x12";
  const char* line5src = "\x74\x1c\x1\x1a\x53\x4c\x3\x1a\x6\xa\x59\x46\xf\x7\xf\xd\x4c\x53\x10\x11\x17\x0\xb";

  char line3[24]; line3[0] = line3src[0];
  char line4[17]; line4[0] = line4src[0];
  char line5[23]; line5[0] = line5src[0];
  ...
  uint8_t w = MAX(canvas_string_width(canvas, line3), MAX(canvas_string_width(canvas, line4), canvas_string_width(canvas, line5)));
  canvas_draw_rframe(canvas, (SCREEN_WIDTH - w) / 2 - paddingW, currY - paddingH - 1, w + paddingW * 2, lineH*3 + paddingH * 2, 3);
