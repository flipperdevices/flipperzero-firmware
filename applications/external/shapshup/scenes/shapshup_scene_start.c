...
void shapshup_scene_start_on_enter(void* context) {
    ...
    if(shapshup_main_view_no_file(view) && !instance->kostilya) {
        instance->kostilya = false;
        scene_manager_next_scene(instance->scene_manager, ShapshupSceneLoadFile);
    } else if(instance->kostilya) {
        instance->kostilya = false;
        scene_manager_stop(instance->scene_manager);
        view_dispatcher_stop(instance->view_dispatcher);
        //scene_manager_next_scene(instance->scene_manager, ShapShupCustomEventTypeExitApp);
    }
}
...
bool shapshup_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == ShapShupCustomEventTypeMenuSelected) {
            ...
            scene_manager_next_scene(instance->scene_manager, ShapshupSceneLoadFile);
            consumed = true;
        } else if(event.event == ShapShupCustomEventTypeExitApp) {
            //exit app
            scene_manager_stop(instance->scene_manager);
            view_dispatcher_stop(instance->view_dispatcher);
            consumed = true;
