...
static int32_t uart_rx_thread(void* ctx) {
    ...
    while(1) {
        ...
        if(evts & rx_done) {
            ...
            if(rx_buf_len > 0) {
                for(i = 0; i < rx_buf_len; i++) switch(dec_buf_len) {
                        /* We're waiting for a sync byte */
                    case 0:
                        if(app->rx_buf[i] == 0x59) app->dec_buf[dec_buf_len++] = app->rx_buf[i];
                        break;
                    ...
                    default:
                        ...
                        app->dec_buf[dec_buf_len++] = app->rx_buf[i];
                        if(dec_buf_len < wait_dec_buf_len) break;
                 ...
                 discard the frame */
                     ...
                     and discard the frame if it isn't */
                            ...
                            is_fw_newer_than_x4 = fw_minor > 4 || (fw_minor == 4 && fw_micro > 0);
