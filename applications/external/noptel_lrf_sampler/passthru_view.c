...
static void vcp_on_cdc_rx(void* ctx) {
    ...
    passthru_model->vcp_rx_buf_len = furi_hal_cdc_receive(
        passthru_vcp_channel, passthru_model->vcp_rx_buf, sizeof(passthru_model->vcp_rx_buf));
...
void passthru_view_enter_callback(void* ctx) {
    ...
    passthru_model->vcp_config = furi_hal_cdc_get_port_settings(passthru_vcp_channel);
...
void passthru_view_draw_callback(Canvas* canvas, void* model) {
    ...
    if(passthru_model->show_traffic_counters) {
        ...
        canvas_draw_str_aligned(canvas, 64, 14, AlignCenter, AlignBottom, passthru_model->spstr1);
        ...
        canvas_draw_str_aligned(canvas, 64, 46, AlignCenter, AlignBottom, passthru_model->spstr1);
