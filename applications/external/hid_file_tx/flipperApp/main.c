...
static void dispatch_view(void* contextd, uint32_t index) {
    ...
    if (index == VIEW_DISPATCHER_SEND || index == VIEW_DISPATCHER_SEND_SINGLE_THREADED) {
        ...
        dialog_file_browser_set_basic_options(
            &browser_options, HEX_VIEWER_APP_EXTENSION, NULL);
        browser_options.hide_ext = false;
        browser_options.base_path = furi_string_get_cstr(browser_path);


        DialogsApp* dialogs = furi_record_open(RECORD_DIALOGS);
        bool res = dialog_file_browser_show(dialogs, selected_path, browser_path, &browser_options);
        ...
        furi_string_set_n(path_copy, selected_path, idx + 1, furi_string_size(selected_path) - idx - 1);
        ...
        while (sent < file_size) {
            ...
            if (to_read == 61) {
                ...
                PartPayloadMessage msg = {
                    .header = {
                        .counter = msgCounter,
                        .messageType = MessagePartPayload
                    },
        ...
        FuriString *filePath = furi_string_alloc_printf("/any/HIDTransfer/%s.raw", metadataMsg->fileName);
        if (!buffered_file_stream_open(fs, furi_string_get_cstr(filePath), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        ...
        while (true) {
            ...
            if (numMsgs % increamentStep == 0) {
                ...
                furi_string_printf(textBoxText, "Receiving %s...\n%d%%", metadataMsg->fileName, percent);
                text_box_set_text(textBoxReceive, furi_string_get_cstr(textBoxText));
            }
...
int32_t hidtransfer_app() {
    ...
    menu_add_item(mainMenu, "Send Client", &I_right_14, VIEW_DISPATCHER_SEND_SINGLE_THREADED, dispatch_view, app);
    menu_add_item(mainMenu, "Send File", &I_right_14, VIEW_DISPATCHER_SEND, dispatch_view, app);
    menu_add_item(mainMenu, "Receive File", &I_left_14, VIEW_DISPATCHER_RECEIVE, dispatch_view, app);
