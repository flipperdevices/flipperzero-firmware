...
MU_TEST_1(test_dirwalk_full, Storage* storage) {
    ...
    mu_check(dir_walk_open(dir_walk, STORAGE_EXT_PATH_PREFIX "/dirwalk"));

    while(dir_walk_read(dir_walk, path, &fileinfo) == DirWalkOK) {
        string_right(path, strlen(STORAGE_EXT_PATH_PREFIX "/dirwalk/"));
...
MU_TEST_SUITE(test_dirwalk_suite) {
    ...
    storage_dirs_create(storage, STORAGE_EXT_PATH_PREFIX "/dirwalk");
    ...
    storage_simply_remove_recursive(storage, STORAGE_EXT_PATH_PREFIX "/dirwalk");
