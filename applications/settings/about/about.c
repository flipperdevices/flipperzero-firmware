...
static DialogMessageButton about_screen_product(DialogsApp* dialogs, DialogMessage* message) {
    ...
    dialog_message_set_text(
        message, furi_string_get_cstr(screen_text), 0, 14, AlignLeft, AlignTop);
...
static DialogMessageButton about_screen_cfw(DialogsApp* dialogs, DialogMessage* message) {
    DialogMessageButton result;

    dialog_message_set_icon(message, &I_ctclcfw_126x50, 1, 1);
    result = dialog_message_show(dialogs, message);
...
static DialogMessageButton about_screen_fw_version(DialogsApp* dialogs, DialogMessage* message) {
    ...
    dialog_message_set_text(message, furi_string_get_cstr(buffer), 0, 9, AlignLeft, AlignTop);
    result = dialog_message_show(dialogs, message);
    furi_string_free(buffer);
...
int32_t about_settings_app(void* p) {
    ...
    view_dispatcher_add_view(view_dispatcher, empty_screen_index, empty_screen_get_view(empty_screen));
