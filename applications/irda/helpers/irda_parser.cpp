...
bool irda_parser_save_signal(FlipperFile* ff, const IrdaAppSignal& signal, const std::string& name) {
    ...
    do {
        ...
        if (signal.is_raw()) {
            ...
            if (!flipper_file_write_uint32(ff, "data", raw_signal.timings, raw_signal.timings_cnt)) break;
        } else {
...
bool irda_parser_is_raw_signal_valid(uint32_t frequency, float duty_cycle, uint32_t timings_cnt) {
    ...
    if((frequency > IRDA_MAX_FREQUENCY) || (frequency < IRDA_MIN_FREQUENCY)) {
        FURI_LOG_E(TAG, "Frequency is out of range (%lX - %lX): %lX", IRDA_MIN_FREQUENCY, IRDA_MAX_FREQUENCY, frequency);
        result = false;
        ...
        FURI_LOG_E(TAG, "Timings amount is out of range (0 - %lX): %lX", MAX_TIMINGS_AMOUNT, timings_cnt);
        result = false;
    }
