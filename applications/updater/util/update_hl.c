...
UpdatePrepareResult update_hl_prepare(const char* manifest_file_path) {
    ...
    if(!update_hl_get_package_dir_name(manifest_file_path, update_folder)) {
        string_clear(update_folder);
        return UpdatePrepareResultManifestPathInvalid;
    }

    Storage* storage = furi_record_open("storage");
    int32_t update_index = update_hl_get_package_index(storage, string_get_cstr(update_folder));
    string_clear(update_folder);
