...
bool update_manifest_init(UpdateManifest* update_manifest, const char* manifest_filename) {
    ...
    do {
        if(!flipper_file_open_existing(flipper_file, manifest_filename)) {
            break;
        }
        ...
        if(!flipper_file_read_uint32(flipper_file, "Target", &update_manifest->target, 1)) {
            break;
        }

        if(!flipper_file_read_string(flipper_file, "Loader", update_manifest->staged_loader_file)) {
            break;
        }

        if(!flipper_file_read_string(
               flipper_file, "Firmware", update_manifest->firmware_dfu_image)) {
            break;
        }

        if(!flipper_file_read_string(flipper_file, "Radio", update_manifest->radio_image)) {
            break;
        }
