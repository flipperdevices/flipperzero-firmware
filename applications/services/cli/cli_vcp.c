...
static void cli_vcp_tx(const uint8_t* buffer, size_t size) {
    ...
    while(size > 0 && vcp->connected) {
        ...
        VCP_DEBUG(TAG, "tx %u", batch_size);

        size -= batch_size;
        buffer += batch_size;
    }

    VCP_DEBUG(TAG, "tx %u end", size);
}
