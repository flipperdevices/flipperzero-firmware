...
static void lfrfid_write_block_t5577_password_and_config_to_EM(LfRfid* app) {
    ...
    snprintf(curr_buf, sizeof(curr_buf), "Page %u  Block %u\nHex: %02X %02X %02X %02X\nDec: %lu", page, block, data_p[0], data_p[1], data_p[2], data_p[3], data);
	...
	bool use_password =  app->extra_options & LfRfidUsePassword;
        ...
        view_dispatcher_switch_to_view(app->view_dispatcher, LfRfidViewPopup);
	if (use_password)
	    t5577_write_page_block_pass_with_start_and_stop(page, block, lock_bit, data, true, current_password/*, false*/);
	else
