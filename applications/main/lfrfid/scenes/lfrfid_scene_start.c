...
bool lfrfid_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
	    ...
	    // Like the NFC app (see comments in `nfc_scene_start.c`),
	    // it is best to save the scene in every branch in case
	    // the user cancels selecting a file.
	    ...
	    scene_manager_set_scene_state(app->scene_manager, LfRfidSceneStart, SubmenuIndexAddManually);
            scene_manager_next_scene(app->scene_manager, LfRfidSceneSaveType);
            consumed = true;
        } else if(event.event == SubmenuIndexExtraActions) {
	    scene_manager_set_scene_state(app->scene_manager, LfRfidSceneStart, SubmenuIndexExtraActions);
