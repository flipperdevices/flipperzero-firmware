...
static bool microel_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        uint16_t balance = (data->block[4].data[6] << 8) | (data->block[4].data[5]);
        ...
        furi_string_cat_printf(parsed_data, "\nCurrent Credit: %d.%02d E ", balance / 100, balance % 100);
        furi_string_cat_printf(parsed_data, "\nPrevius Credit: %d.%02d E ", previus_balance / 100, previus_balance % 100);
