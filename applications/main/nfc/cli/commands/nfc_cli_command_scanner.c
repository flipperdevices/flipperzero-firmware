...
static NfcCliActionContext* nfc_cli_scanner_alloc(Nfc* nfc) {
    furi_assert(nfc);
...
static void nfc_cli_scanner_free(NfcCliActionContext* ctx) {
    furi_assert(ctx);
...
///TODO: Here context must be from alloc callback called before to initialize context
static void nfc_cli_scanner_execute(PipeSide* pipe, void* context) {
    NfcCliScanner* cli_scanner = context;
    //    NfcCliContext* instance = context;

    //   NfcCliScanner* cli_scanner = nfc_cli_scanner_alloc(instance->nfc);
...
const NfcCliActionDescriptor scanner_action = {
    ...
    .alloc = nfc_cli_scanner_alloc,
    .free = nfc_cli_scanner_free,
};
