...
bool nfc_scene_read_mf_classic_worker_callback(NfcWorkerEvent event, void* context) {
    Nfc* nfc = context;
    bool consumed = false;
...
void nfc_scene_read_mf_classic_set_state(Nfc* nfc, NfcSceneReadState state) {
...
void nfc_scene_read_mf_classic_on_enter(void* context) {
    Nfc* nfc = context;

    nfc_device_clear(nfc->dev);
    // Setup view
    nfc_scene_read_mf_classic_set_state(nfc, NfcSceneReadStateDetecting);
    view_dispatcher_switch_to_view(nfc->view_dispatcher, NfcViewPopup);
    // Start worker
    nfc_worker_start(
        nfc->worker,
        NfcWorkerStateReadMfClassic,
        &nfc->dev->dev_data,
        nfc_scene_read_mf_classic_worker_callback,
        nfc);

    nfc_blink_read_start(nfc);
}

bool nfc_scene_read_mf_classic_on_event(void* context, SceneManagerEvent event) {
    Nfc* nfc = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == NfcWorkerEventReadMfClassicDone) {
            ...
            nfc_scene_read_mf_classic_set_state(nfc, NfcSceneReadStateReading);
...
void nfc_scene_read_mf_classic_on_exit(void* context) {
    Nfc* nfc = context;
