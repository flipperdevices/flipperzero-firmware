...
MfDesfireApplication* nfc_scene_mf_desfire_app_get_app(NfcApp* nfc) {
    uint32_t app_idx = scene_manager_get_scene_state(nfc->scene_manager, NfcSceneMfDesfireApp) >>
                       1;
    MfDesfireApplication* app = nfc->nfc_dev_data.mf_desfire_data.app_head;
...
void nfc_scene_mf_desfire_app_on_enter(void* context) {
    NfcApp* nfc = context;
    MfDesfireApplication* app = nfc_scene_mf_desfire_app_get_app(nfc);
    if(!app) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == NfcCustomEventViewExit) {
            ...
            if(event.event == SubmenuIndexAppInfo) {
                ...
                MfDesfireFile* file = app->file_head;
