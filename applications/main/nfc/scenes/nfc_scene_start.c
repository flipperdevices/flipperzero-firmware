...
bool nfc_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
	    ...
	    scene_manager_set_scene_state(nfc->scene_manager, NfcSceneStart, SubmenuIndexDetectReader);
	    ...
	    // Although the scene state can be saved outside these
	    // branches, if the user cancels loading a file, the state
	    // isn't actually saved. Therefore, the best approach is
	    // to save the scene in every branch.
	    ...
	    scene_manager_set_scene_state(nfc->scene_manager, NfcSceneStart, SubmenuIndexExtraAction);
