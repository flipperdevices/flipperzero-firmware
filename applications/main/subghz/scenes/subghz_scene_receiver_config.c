...
static void subghz_scene_receiver_config_set_frequency(VariableItem* item) {
    ...
    if(subghz_hopper_get_state(subghz->txrx) == SubGhzHopperStateOFF) {
        ...
        subghz->txrx->preset->frequency = subghz_setting_get_frequency(subghz->txrx->setting, index);
    } else {
...
static void subghz_scene_receiver_config_set_preset(VariableItem* item) {
    ...
    uint8_t index = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(
        item, subghz_setting_get_preset_name(subghz->txrx->setting, index));
    subghz_preset_init(
        subghz->txrx,
        subghz_setting_get_preset_name(subghz->txrx->setting, index),
    ...
    if(hopping_value[index] == SubGhzHopperStateOFF) {
        ...
        variable_item_set_current_value_text(
            (VariableItem*)scene_manager_get_scene_state(
                subghz->scene_manager, SubGhzSceneReceiverConfig),
            text_buf);
        subghz->txrx->preset->frequency = subghz_setting_get_default_frequency(subghz->txrx->setting);
        variable_item_set_current_value_index(
            (VariableItem*)scene_manager_get_scene_state(
                subghz->scene_manager, SubGhzSceneReceiverConfig),
