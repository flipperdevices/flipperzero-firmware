...
static void spectrum_analyzer_render_callback(Canvas* const canvas, void* ctx) {
    ...
    if (model->max_rssi > PEAK_THRESHOLD) {
        ...
        // Label
        char temp_str[36];
        snprintf(temp_str, 36, "Peak: %3.2f Mhz %3.1f dbm", ((float)(model->channel0_frequency + (model->max_rssi_channel * model->spacing)) / 1000000), model->max_rssi);
...
void spectrum_analyzer_calculate_frequencies( SpectrumAnalyzerModel* model) {
    ...
    model->channel0_frequency = model->center_freq * 1000000 - ( model->spacing * ((NUM_CHANNELS / 2) + 1 ));
    ...
    FURI_LOG_D("Spectrum", "ch[0]: %lu - ch[%u]: %lu", model->channel0_frequency, NUM_CHANNELS - 1, model->channel0_frequency + ((NUM_CHANNELS - 1) * model->spacing));
...
SpectrumAnalyzer* spectrum_analyzer_alloc() {
    ...
    spectrum_analyzer_worker_set_callback(instance->worker, spectrum_analyzer_worker_callback, instance);
    ...
    while(osMessageQueueGet(spectrum_analyzer->event_queue, &input, NULL, osWaitForever) == osOK) {
        ...
        switch(input.key) {
            ...
            case InputKeyRight: 
                ...
                spectrum_analyzer_worker_set_frequencies(spectrum_analyzer->worker, model->channel0_frequency, model->spacing, model->width);
                break; 
            case InputKeyLeft:  
                model->center_freq -= hstep;
                spectrum_analyzer_calculate_frequencies(model);
