...
static void nfc_worker_listener_pass_col_res(Nfc* instance, uint8_t* rx_data, uint16_t rx_bits) {
    furi_crash(instance->col_res_status != Iso14443_3aColResStatusDone);
...
void nfc_start(Nfc* instance, NfcEventCallback callback, void* context) {
    furi_crash(instance);
    furi_crash(instance->worker_thread == NULL);

    if(instance->mode == NfcModeListener) {
        furi_crash(listener_queue == NULL);
        // Check that poller didn't start
        furi_crash(poller_queue == NULL);
    } else {
        furi_crash(poller_queue == NULL);
        // Check that poller is started after listener
        furi_crash(listener_queue);
    }
...
void nfc_stop(Nfc* instance) {
    furi_crash(instance);
    furi_crash(instance->worker_thread);
...
NfcError nfc_listener_tx(Nfc* instance, const BitBuffer* tx_buffer) {
    furi_crash(instance);
    furi_crash(poller_queue);
    furi_crash(listener_queue);
    furi_crash(tx_buffer);
...
NfcError
    ...
    bit_buffer_write_bytes(tx_buffer, message.data.data, bit_buffer_get_size_bytes(tx_buffer));
    // Tx
    furi_crash(furi_message_queue_put(listener_queue, &message, FuriWaitForever) == FuriStatusOk);
    // Rx
