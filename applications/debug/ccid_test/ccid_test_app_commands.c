/**
 * @file ccid_test_app_commands.c
 * @brief CCID Test Application Commands
 * 
 * Custom test commands using CLA=0x01.
 * Standard ISO7816 commands (CLA=0x00) are handled by the default handler.
 */
...
static void handle_instruction_01(ISO7816_Response_APDU* response_apdu) {
...
static void handle_instruction_02(
    uint8_t p1,
    uint8_t p2,
    uint16_t lc,
    uint16_t le,
...
//response SW1=0x90, SW2=0x00
static void handle_instruction_03(
    uint8_t p1,
    uint8_t p2,
    uint16_t lc,
    ISO7816_Response_APDU* response_apdu) {
...
static void handle_instruction_04(
    uint8_t p1,
    uint8_t p2,
    uint16_t lc,
    uint16_t le,
...
bool iso7816_process_command(
    ...
    // Only handle test class (CLA=0x01)
    // Let default handler process standard ISO7816 commands (CLA=0x00)
    if(command_apdu->CLA != 0x01) {
        return false;  // Not handled - use default handler
    }
