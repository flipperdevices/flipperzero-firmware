...
#define M_OPL_WSReceiverMenuItemArray_t() \
    ARRAY_OPLIST(WSReceiverMenuItemArray, M_POD_OPLIST)
...
void ws_view_receiver_set_lock(WSReceiver* ws_receiver, WSLock lock) {
    ...
    if(lock == WSLockOn) {
        ...
        with_view_model(
            ws_receiver->view, (WSReceiverModel * model) {
    ...
    with_view_model(
        ws_receiver->view, (WSReceiverModel * model) {
            WSReceiverMenuItem* item_menu =
                WSReceiverMenuItemArray_push_raw(model->history->data);
...
static void ws_view_receiver_timer_callback(void* context) {
    ...
    if(ws_receiver->lock_count < UNLOCK_CNT) {
        ws_receiver->callback(
            WSCustomEventViewReceiverOffDisplay, ws_receiver->context);
    } else {
...
bool ws_view_receiver_input(InputEvent* event, void* context) {
    ...
    if(ws_receiver->lock == WSLockOn) {
        ...
        if(ws_receiver->lock_count >= UNLOCK_CNT) {
             ws_receiver->callback(
                WSCustomEventViewReceiverUnlock, ws_receiver->context);
            with_view_model(
                ws_receiver->view, (WSReceiverModel * model) {
    ...
    if(event->key == InputKeyBack && event->type == InputTypeShort) {
        ...
        with_view_model(
            ws_receiver->view, (WSReceiverModel * model) {
                if(model->history_item != 0) {
                    ws_receiver->callback(
                        WSCustomEventViewReceiverOK, ws_receiver->context);
                }
                return false;
            });
    }
...
WSReceiver* ws_view_receiver_alloc() {
    ...
    ws_receiver->lock_count = 0;
    view_allocate_model(
        ws_receiver->view, ViewModelTypeLocking, sizeof(WSReceiverModel));
