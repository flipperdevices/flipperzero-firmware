...
static void spi_mem_verify_process(SPIMemWorker* worker) {
    ...
    while(true) {
        ...
        if((offset + SPI_MEM_FILE_BUFFER_SIZE) >= chip_size) break;
        if(!spi_mem_tools_read_block_data(
               worker->chip_info, offset, data_buffer_chip, SPI_MEM_FILE_BUFFER_SIZE)) {
            spi_mem_run_worker_callback(worker, SPIMemCustomEventWorkerChipReadFail);
            success = false;
            break;
        }
        if(!spi_mem_file_read_block(worker->cb_ctx, data_buffer_file, SPI_MEM_FILE_BUFFER_SIZE)) {
            spi_mem_run_worker_callback(worker, SPIMemCustomEventWorkerReadFileFail);
            success = false;
            break;
        }
        if(memcmp(data_buffer_chip, data_buffer_file, SPI_MEM_FILE_BUFFER_SIZE) != 0) {
