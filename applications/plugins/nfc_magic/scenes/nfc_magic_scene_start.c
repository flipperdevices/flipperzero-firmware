...
bool nfc_magic_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
	    ...
	    scene_manager_set_scene_state(nfc_magic->scene_manager, NfcMagicSceneStart, SubmenuIndexCheck);
	    ...
	    // loading a file.
	    scene_manager_set_scene_state(nfc_magic->scene_manager, NfcMagicSceneStart, SubmenuIndexWriteGen1A);
