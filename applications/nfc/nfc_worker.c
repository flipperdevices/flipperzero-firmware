...
void nfc_worker_detect(NfcWorker* nfc_worker) {
    ...
    NfcMessage message = {.nfc_detect_model.found = false};

    while(nfc_worker->state == NfcWorkerStateDetect) {
        message.nfc_detect_model.found = false;
        if(api_hal_nfc_detect(&dev_list, &dev_cnt, 1000, true)) {
            // Process first found device
            dev = &dev_list[0];
            message.nfc_detect_model.found = true;
            message.nfc_detect_model.data.uid_len = dev->nfcidLen;
            memcpy(message.nfc_detect_model.data.uid, dev->nfcid, dev->nfcidLen);

            if(dev->type == RFAL_NFC_LISTEN_TYPE_NFCA) {
                message.nfc_detect_model.data.device = NfcDeviceNfca;
                message.nfc_detect_model.data.atqa[0] = dev->dev.nfca.sensRes.anticollisionInfo;
                message.nfc_detect_model.data.atqa[1] = dev->dev.nfca.sensRes.platformInfo;
                message.nfc_detect_model.data.sak = dev->dev.nfca.selRes.sak;
                // TODO check protocols

            } else if(dev->type == RFAL_NFC_LISTEN_TYPE_NFCB) {
                message.nfc_detect_model.data.device = NfcDeviceNfcb;
            } else if(dev->type == RFAL_NFC_LISTEN_TYPE_NFCF) {
                message.nfc_detect_model.data.device = NfcDeviceNfcf;
...
void nfc_worker_read_emv(NfcWorker* nfc_worker) {
    ...
    NfcMessage message = {.nfc_emv.found = false};
        ...
        if(api_hal_nfc_detect(&dev_list, &dev_cnt, 1000, false)) {
            ...
            if(dev_list[0].rfInterface == RFAL_NFC_INTERFACE_ISODEP) {
                ...
                if(emv_decode_select_app_response(rx_buff, *rx_len, &emv_app)) {
                    ...
                    memcpy(message.nfc_emv.emv_data.name, emv_app.name, sizeof(emv_app.name));
                } else {
                ...
                if(emv_decode_get_proc_opt(rx_buff, *rx_len, &emv_app)) {
                    ...
                    message.nfc_emv.found = true;
                    memcpy(
                        message.nfc_emv.emv_data.number,
                        emv_app.card_number,
...
void nfc_worker_read_mf_ultralight(NfcWorker* nfc_worker) {
    ...
    NfcMessage message = {.nfc_mifare_ul.found = false};
               ...
               mf_ul_check_card_type(
                ...
                if(err == ERR_NONE) {
                    ...
                    if(!api_hal_nfc_detect(&dev_list, &dev_cnt, 1000, false)) {
                        ...
                        message.nfc_mifare_ul.found = false;
                        continue;
                    }
                } else {
                    FURI_LOG_E(
                ...
                message.nfc_mifare_ul.found = true;
                memcpy(
                    message.nfc_mifare_ul.nfc_data.uid,
                    dev_list[0].dev.nfca.nfcId1,
                    sizeof(message.nfc_mifare_ul.nfc_data.uid));
                memcpy(message.nfc_mifare_ul.nfc_mf_ul_data.man_block, mf_ul_read.dump, 4 * 3);
                memcpy(message.nfc_mifare_ul.nfc_mf_ul_data.otp, &mf_ul_read.dump[4 * 3], 4);
