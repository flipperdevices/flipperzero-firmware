...
void animation_manager_unload_and_stall_animation(AnimationManager* animation_manager) {
    ...
    furi_assert((animation_manager->state == AnimationManagerStateIdle) || (animation_manager->state == AnimationManagerStateBlocked));
...
void animation_manager_load_and_continue_animation(AnimationManager* animation_manager) {
    ...
    furi_assert(!animation_manager->current_animation);
    furi_assert(string_size(animation_manager->freezed_animation_name));
    furi_assert((animation_manager->state == AnimationManagerStateFreezedIdle) || (animation_manager->state == AnimationManagerStateFreezedBlocked));
        ...
        if (!blocked) {
            ...
            if (restore_animation) {
                ...
                if (animation_manager->freezed_animation_time_left) {
                    ...
                    const BubbleAnimation* animation = animation_storage_get_bubble_animation(animation_manager->current_animation);
                    osTimerStart(animation_manager->idle_animation_timer, animation->duration * 1000);
                }
            } else {
                FURI_LOG_E(TAG, "Failed to restore \'%s\'", string_get_cstr(animation_manager->freezed_animation_name));
            }
        }
    } else {
