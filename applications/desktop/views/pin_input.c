...
//    canvas_clear(canvas);    // dbg_
    ...
    if (model->secondary_str) {
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str(canvas, model->secondary_str_x, model->secondary_str_y, model->secondary_str);
    }
}
...
DesktopViewPinInput* desktop_view_pin_input_alloc(void) {
    ...
    pin_input->timer = xTimerCreate("", pdMS_TO_TICKS(NO_ACTIVITY_TIMEOUT), pdFALSE, pin_input, desktop_view_pin_input_timer_callback);
