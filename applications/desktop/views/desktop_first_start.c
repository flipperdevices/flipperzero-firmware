...
static void desktop_first_start_draw(Canvas* canvas, void* model) {
    ...
    if(m->page == 0) {
        ...
        canvas_draw_icon(
            canvas, width - 57, height - 45 + STATUS_BAR_Y_SHIFT, &I_DolphinFirstStart3_57x48);
        elements_multiline_text_framed(
            canvas, 0, 16 + STATUS_BAR_Y_SHIFT, "Kickstarter\ncampaign\nwas INSANE! >");
    } else if(m->page == 4) {
        canvas_draw_icon(
            canvas, width - 67, height - 51 + STATUS_BAR_Y_SHIFT, &I_DolphinFirstStart4_67x53);
        elements_multiline_text_framed(
        ...
        canvas_draw_icon(
            canvas, width - 61, height - 51 + STATUS_BAR_Y_SHIFT, &I_DolphinFirstStart7_61x51);
        elements_multiline_text_framed(
            canvas, 0, 13 + STATUS_BAR_Y_SHIFT, "As long as\nyou read, write\nand emulate >");
    } else if(m->page == 8) {
        canvas_draw_icon(
            canvas, width - 56, height - 51 + STATUS_BAR_Y_SHIFT, &I_DolphinFirstStart8_56x51);
        elements_multiline_text_framed(
            canvas,
            0,
            13 + STATUS_BAR_Y_SHIFT,
