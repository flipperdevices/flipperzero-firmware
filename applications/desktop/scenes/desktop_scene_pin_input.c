...
static void desktop_scene_pin_input_set_timer(Desktop* desktop, bool enable, TickType_t new_period) {
    furi_assert(desktop);

    DesktopScenePinInputState* state = (DesktopScenePinInputState*) scene_manager_get_scene_state(desktop->scene_manager, DesktopScenePinInput);
    furi_assert(state);
    if (enable) {
...
static void desktop_scene_pin_input_done_callback(const PinCode* pin_code, void* context) {
    ...
    if (pins_are_equal(&desktop->settings.pin_code, pin_code)) {
        ...
        view_dispatcher_send_custom_event(desktop->view_dispatcher, DesktopPinInputEventUnlockFailed);
    }
}
...
static void desktop_scene_pin_input_timer_callback(TimerHandle_t timer) {
    ...
    view_dispatcher_send_custom_event(desktop->view_dispatcher, DesktopPinInputEventResetWrongPinLabel);
}
...
void desktop_scene_pin_input_on_enter(void* context) {
    ...
    desktop_view_pin_input_set_back_callback(desktop->pin_input_view, desktop_scene_pin_input_back_callback);
    desktop_view_pin_input_set_timeout_callback(desktop->pin_input_view, desktop_scene_pin_input_back_callback);
    desktop_view_pin_input_set_done_callback(desktop->pin_input_view, desktop_scene_pin_input_done_callback);

    DesktopScenePinInputState* state = furi_alloc(sizeof(DesktopScenePinInputState));
    state->timer = xTimerCreate("", 10000, pdFALSE, desktop, desktop_scene_pin_input_timer_callback);
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case DesktopPinInputEventUnlockFailed:
            ...
            if (pin_timeout > 0) {
                ...
                desktop_view_pin_input_set_label_secondary(desktop->pin_input_view, 25, 25, "Wrong PIN try again:");
        ...
        case DesktopPinInputEventBack:
            scene_manager_search_and_switch_to_previous_scene(desktop->scene_manager, DesktopSceneLocked);
            consumed = true;
            break;
        }
    }
