...
static bool _fs_init(SdFsInfo* _fs_info) {
    bool result = true;
...
static bool _fs_lock(SdFsInfo* fs_info) {
    api_hal_power_insomnia_enter();
    return (osMutexAcquire(fs_info->mutex, osWaitForever) == osOK);
}

static bool _fs_unlock(SdFsInfo* fs_info) {
    api_hal_power_insomnia_exit();
    return (osMutexRelease(fs_info->mutex) == osOK);
}

static SDError
    _get_filedata(SdFsInfo* fs_info, File* file, FileData** filedata, FiledataFilter filter) {
    SDError error = SD_OK;
    _fs_lock(fs_info);
...
static SDError _get_file(SdFsInfo* fs_info, File* file, FileData** filedata) {
    return _get_filedata(fs_info, file, filedata, FDF_FILE);
}

static SDError _get_dir(SdFsInfo* fs_info, File* file, FileData** filedata) {
    return _get_filedata(fs_info, file, filedata, FDF_DIR);
}

static SDError _get_any(SdFsInfo* fs_info, File* file, FileData** filedata) {
    return _get_filedata(fs_info, file, filedata, FDF_ANY);
}

static SDError _fs_status(SdFsInfo* fs_info) {
    SDError result;
...
static FS_Error _fs_parse_error(SDError error) {
    FS_Error result;
    switch(error) {
    case SD_OK:
        result = FSE_OK;
...
bool sd_storage_file_close(File* file) {
...
uint16_t sd_storage_file_read(File* file, void* buff, uint16_t const bytes_to_read) {
    FileData* filedata = NULL;
...
uint16_t sd_storage_file_write(File* file, const void* buff, uint16_t const bytes_to_write) {
    FileData* filedata = NULL;
...
bool sd_storage_file_seek(File* file, const uint32_t offset, const bool from_start) {
    FileData* filedata = NULL;
...
uint64_t sd_storage_file_tell(File* file) {
    FileData* filedata = NULL;
    uint64_t position = 0;
    file->internal_error_id = _get_file(fs_info, file, &filedata);
...
bool sd_storage_file_truncate(File* file) {
    FileData* filedata = NULL;
...
// Flush cached data
bool sd_storage_file_sync(File* file) {
    FileData* filedata = NULL;
...
// Get size
uint64_t sd_storage_file_size(File* file) {
    FileData* filedata = NULL;
    uint64_t size = 0;
    file->internal_error_id = _get_file(fs_info, file, &filedata);
...
// Test EOF
bool sd_storage_file_eof(File* file) {
    FileData* filedata = NULL;
    bool eof = true;
    file->internal_error_id = _get_file(fs_info, file, &filedata);
...
// Open directory
bool sd_storage_dir_open(File* file, const char* path) {
    SDDir* sd_dir = NULL;
...
// Close directory
bool sd_storage_dir_close(File* file) {
    FileData* filedata = NULL;
...
bool sd_storage_dir_read(File* file, FileInfo* fileinfo, char* name, const uint16_t name_length) {
    FileData* filedata = NULL;
    file->internal_error_id = _get_dir(fs_info, file, &filedata);
...
bool sd_storage_dir_rewind(File* file) {
    FileData* filedata = NULL;
    file->internal_error_id = _get_dir(fs_info, file, &filedata);
...
FS_Error sd_storage_common_info(
    const char* path,
    FileInfo* fileinfo,
    char* name,
...
FS_Error sd_storage_common_remove(const char* path) {
    SDError fresult = _fs_status(fs_info);
...
// Rename file/dir
// File/dir must not be opened, or the FAT volume can be collapsed. FF_FS_LOCK fix that.
FS_Error sd_storage_common_rename(const char* old_path, const char* new_path) {
    SDError fresult = _fs_status(fs_info);
...
FS_Error sd_storage_common_mkdir(const char* path) {
    SDError fresult = _fs_status(fs_info);
...
FS_Error sd_storage_get_fs_info(uint64_t* total_space, uint64_t* free_space) {
    SDError fresult = _fs_status(fs_info);
...
const char* sd_storage_error_get_desc(FS_Error error_id) {
    const char* result;
    switch(error_id) {
    case(FSE_OK):
        result = "OK";
...
const char* sd_storage_error_get_internal_desc(uint32_t internal_error_id) {
    const char* result;
