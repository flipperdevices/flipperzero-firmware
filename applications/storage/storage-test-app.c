...
int32_t storage_app_test(void* p) {
    ...
    // open
    result = api->file.open(api->context, &file, "/ext/test.txt", FSAM_WRITE, FSOM_CREATE_ALWAYS);
    if(result) {
    ...
    // open
    result = api->file.open(api->context, &file, "/ext/test.txt", FSAM_READ, FSOM_OPEN_EXISTING);
    if(result) {
        FURI_LOG_I(TAG, "file opened");
    } else {
        FURI_LOG_E(TAG, "file cannot be open, %d", file.error_id);
