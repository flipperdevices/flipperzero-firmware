...
void subghz_receiver_draw(Canvas* canvas, SubghzReceiverModel* model) {
    ...
    if(model->protocol && model->protocol->to_save_string && strcmp(model->protocol->name, "KeeLoq")) {
...
bool subghz_receiver_input(InputEvent* event, void* context) {
    ...
    with_view_model(
        subghz_receiver->view, (SubghzReceiverModel * model) {
            can_be_saved = (model->protocol && model->protocol->to_save_string && strcmp(model->protocol->name, "KeeLoq"));
            return false;
        });
