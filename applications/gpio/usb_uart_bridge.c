...
static void usb_uart_on_irq_cb(UartIrqEvent ev, uint8_t data) {
    ...
    if(ev == UartIrqEventRXNE) {
        ...
        if(ret > USB_PKT_LEN) osEventFlagsSet(usb_uart->events, WorkerEvtRxDone);

    } else if(ev == UartIrqEventIDLE)
        osEventFlagsSet(usb_uart->events, WorkerEvtRxDone);
}
...
static int32_t usb_uart_worker(void* context) {
    ...
    osEventFlagsSet(usb_uart->events, WorkerEvtCdcRx);
