...
static void usb_uart_vcp_deinit(uint8_t vcp_ch) {
    furi_hal_cdc_set_callbacks(vcp_ch, NULL);
    if(vcp_ch == 0) furi_hal_vcp_enable();
}

static void usb_uart_serial_init(uint8_t uart_ch) {
    if(uart_ch == FuriHalUartIdUSART1) {
        furi_hal_console_disable();
    } else if(uart_ch == FuriHalUartIdLPUART1) {
        furi_hal_uart_init(uart_ch, 115200);
    }
    furi_hal_uart_set_irq_cb(uart_ch, usb_uart_on_irq_cb);
}

static void usb_uart_serial_deinit(uint8_t uart_ch) {
    furi_hal_uart_set_irq_cb(uart_ch, NULL);
    if(uart_ch == FuriHalUartIdUSART1)
...
static void usb_uart_set_baudrate(uint32_t baudrate) {
    if(baudrate != 0) {
...
static void usb_uart_update_ctrl_lines() {
    ...
    while(1) {
        ...
        if(events & WorkerEvtCfgChange) {
            if(usb_uart->cfg.vcp_ch != usb_uart->cfg_new.vcp_ch) {
                ...
                usb_uart_vcp_deinit(usb_uart->cfg.vcp_ch);
                usb_uart_vcp_init(usb_uart->cfg_new.vcp_ch);
            ...
            if(usb_uart->cfg.uart_ch != usb_uart->cfg_new.uart_ch) {
                ...
                usb_uart_serial_deinit(usb_uart->cfg.uart_ch);
                usb_uart_serial_init(usb_uart->cfg_new.uart_ch);
            ...
            if(usb_uart->cfg.baudrate != usb_uart->cfg_new.baudrate) {
                usb_uart_set_baudrate(usb_uart->cfg_new.baudrate);
        ...
        if(events & WorkerEvtLineCfgSet) {
            if(usb_uart->cfg.baudrate == 0) usb_uart_set_baudrate(usb_uart->cfg.baudrate);
        }
...
void usb_uart_enable(UsbUartConfig* cfg) {
    if(running == false) {
        ...
        furi_thread_set_context(usb_uart->thread, &usb_uart->cfg_new);
