...
#include <sw_digital_protocol/sw_usart.h>
...
#define SW_USART_GPIO_PIN_TX (gpio_ext_pb2)
...
typedef struct {
    ...
    // float temp_celsius;
    // bool has_device;
} ExampleSwUsartContext;

// /*************** 1-Wire Communication and Processing *****************/

// /* Commands the thermometer to begin measuring the temperature. */
// static void example_sw_usart_request_temperature(ExampleSwUsartContext* context) {
...
// static void example_sw_usart_read_temperature(ExampleSwUsartContext* context) {
...
static int32_t example_sw_usart_reader_thread_callback(void* ctx) {
    ExampleSwUsartContext* context = ctx;

    UNUSED(context);

    for(;;) {
        // /* Tell the termometer to start measuring the temperature. The process may take up to 750ms. */
        // example_sw_usart_request_temperature(context);
...
static void example_sw_usart_draw_callback(Canvas* canvas, void* ctx) {
    ExampleSwUsartContext* context = ctx;
    ...
    canvas_draw_str_aligned(canvas, middle_x, 12, AlignCenter, AlignBottom, "Software USART Demo");
    canvas_draw_line(canvas, 0, 16, 128, 16);

    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str_aligned(canvas, middle_x, 30, AlignCenter, AlignBottom, "Connnect device to:");

    snprintf(
        text_store,
        TEXT_STORE_SIZE,
        "TX GPIO pin %ld",
        furi_hal_resources_get_ext_pin_number(&SW_USART_GPIO_PIN_TX));
    canvas_draw_str_aligned(canvas, middle_x, 42, AlignCenter, AlignBottom, text_store);
    snprintf(
        text_store,
        TEXT_STORE_SIZE,
        "RX GPIO pin %ld",
        furi_hal_resources_get_ext_pin_number(&SW_USART_GPIO_PIN_RX));
    canvas_draw_str_aligned(canvas, middle_x, 54, AlignCenter, AlignBottom, text_store);
...
static void example_sw_usart_input_callback(InputEvent* event, void* ctx) {
    ExampleSwUsartContext* context = ctx;
    furi_message_queue_put(context->event_queue, event, FuriWaitForever);
}

/* Starts the reader thread and handles the input */
static void example_sw_usart_run(ExampleSwUsartContext* context) {
...
static ExampleSwUsartContext* example_sw_usart_context_alloc() {
    ExampleSwUsartContext* context = malloc(sizeof(ExampleSwUsartContext));

    context->view_port = view_port_alloc();
    view_port_draw_callback_set(context->view_port, example_sw_usart_draw_callback, context);
    view_port_input_callback_set(context->view_port, example_sw_usart_input_callback, context);
    ...
    furi_thread_set_callback(context->reader_thread, example_sw_usart_reader_thread_callback);
...
static void example_sw_usart_context_free(ExampleSwUsartContext* context) {
    view_port_enabled_set(context->view_port, false);
    gui_remove_view_port(context->gui, context->view_port);

    // onewire_host_free(context->onewire);

    sw_usart_free(context->sw_usart);
...
int32_t example_sw_usart_main(void* p) {
    UNUSED(p);

    /* Allocate all of the necessary structures */
    ExampleSwUsartContext* context = example_sw_usart_context_alloc();

    /* Start the applicaton's main loop. It won't return until the application was requested to exit. */
    example_sw_usart_run(context);

    /* Release all unneeded resources */
    example_sw_usart_context_free(context);
