...
void dolphin_view_first_start_draw(Canvas* canvas, void* model) {
    ...
    if(m->page == 0) {
        ...
        canvas_draw_icon_name(canvas, GUI_MAIN_WIDTH-57, GUI_MAIN_HEIGHT-48, I_DolphinFirstStart3_57x48);
        elements_multiline_text(canvas, 5, 20, "Kickstarter\ncampaign\nwas INSANE! >");
    } else if(m->page == 4) {
        canvas_draw_icon_name(canvas, GUI_MAIN_WIDTH-67, GUI_MAIN_HEIGHT-53, I_DolphinFirstStart4_67x53);
        elements_multiline_text(canvas, 5, 20, "Now\nallow me\nto introduce\nmyself >");
    } else if(m->page == 5) {
        canvas_draw_icon_name(canvas, 0, GUI_MAIN_HEIGHT-53, I_DolphinFirstStart5_45x53);
        elements_multiline_text(canvas, 50, 20, "I am Flipper,\ncyberdolphin\nliving in your\npocket >");
    } else if(m->page == 6) {
        canvas_draw_icon_name(canvas, 0, GUI_MAIN_HEIGHT-54, I_DolphinFirstStart6_58x54);
        elements_multiline_text(canvas, 63, 20, "I can grow\n smart'n'cool\nif you use me\noften >");
    } else if(m->page == 7) {
        canvas_draw_icon_name(canvas, GUI_MAIN_WIDTH-61, GUI_MAIN_HEIGHT-51, I_DolphinFirstStart7_61x51);
        elements_multiline_text(canvas, 5, 20, "As long as\nyou read, write\nand emulate >");
    } else if(m->page == 8) {
        canvas_draw_icon_name(canvas, GUI_MAIN_WIDTH-56, GUI_MAIN_HEIGHT-51, I_DolphinFirstStart8_56x51);
        elements_multiline_text(canvas, 5, 20, "You can check\nmy level and\nmood in the\nPassport menu >");
    }
}
