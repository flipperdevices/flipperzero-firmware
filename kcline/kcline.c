...
const uint16_t MAX_ANIMATION_TICKS = 10; // animation occurs when this is >= MAX_ANIMATIONS_TICKS / 2
...
static void reset_bits(App* app) {
	...
	for (uint8_t i = 0; i < NUM_BITS; i++) {
		...
		if (i == 0) { app->game.bits[i].enabled = false; } else { app->game.bits[i].enabled = true; }
...
static void draw_scene_scene_about (Canvas* canvas, App* app) {
		...
		uint8_t nrows = MAX_ROWS / item_h; // Calculate the number of rows this font can display on screen
		...
		uint8_t row = 10, row_i = 0; // row = starting row to print on canvas, row_i is the current "text row" we are on in the loop
...
static void draw_scene_scene_menu (Canvas* canvas, App* app) {
	...
	for (uint8_t i = 0; i < nitems; ++i) {
		canvas_set_color(canvas, ColorBlack);
	  	if (i == selectedIndex) {
			canvas_draw_box(canvas, MAX_COLS/2-max_width/2, MAX_ROWS/2-item_h*nitems/2+i*item_h, max_width, item_h);
	  	}
	  	canvas_set_color(canvas, i == selectedIndex ? ColorWhite : ColorBlack);
	  	canvas_draw_str_aligned(canvas, MAX_COLS/2, MAX_ROWS/2-item_h*nitems/2+i*item_h+item_h/2, AlignCenter, AlignCenter, MAIN_MENU[i].text);
	}
	canvas_set_color(canvas, ColorBlack);
	canvas_draw_rframe(canvas, MAX_COLS/2-max_width/2-DLT_FRAME, MAX_ROWS/2-item_h*nitems/2-DLT_FRAME, max_width+DLT_FRAME*2, item_h*nitems+DLT_FRAME*2, 2);
...
static void draw_scene_scene_died (Canvas* canvas, App* app) {
	...
	for (uint8_t i = 0; i < KC_LIVES; i++) {
		if (i < app->game.lives) { canvas_draw_icon(canvas, col + (i * 10) + 2, row,  &I_kc1); }
		if (i >= app->game.lives) {
			//FURI_LOG_D(TAG, "Lives: %d, i: %d", app->game.lives, i);
			canvas_draw_line(canvas, col + (i * 10) + 2, row + KC_HEIGHT - 1, col + (i * 10) + 2 + KC_WIDTH, row);
			canvas_draw_line(canvas, col + (i * 10) + 2, row, col + (i * 10) + 2 + KC_WIDTH, row + KC_HEIGHT - 1);
		}
	}
	...
	canvas_draw_rframe(canvas, 
		    upperLeftCol, 
		    upperLeftRow,
		    bottomRightCol, 
		    bottomRightRow, 
		    2);
	canvas_draw_str_aligned(canvas, MAX_COLS / 2, MAX_ROWS - 10, AlignCenter, AlignCenter, "< Press OK >");
...
static void draw_scene_scene_lose (Canvas* canvas, App* app) {
    ...
    canvas_draw_str_aligned(canvas, MAX_COLS/2, MAX_ROWS/2 - h/2 + paddingV, AlignCenter, AlignCenter, GAME_OVER_MESSAGE_LINE_1);
    canvas_draw_str_aligned(canvas, MAX_COLS/2, (MAX_ROWS/2) + paddingV, AlignCenter, AlignCenter, score_message);	
...
static void draw_scene_scene_game (Canvas* canvas, App* app) {
	...
	canvas_set_bitmap_mode(canvas, true); // if not true, i think we lose transparency on loaded pngs
	...
	else { canvas_draw_icon(canvas, app->game.kc.col, app->game.kc.row, &I_kc2); }

	// Draw Ghost
	if (app->game.ticks < MAX_ANIMATION_TICKS / 2) { 
		canvas_draw_icon(canvas, app->game.ghost.col, app->game.ghost.row, &I_ghost1); }
	else { canvas_draw_icon(canvas, app->game.ghost.col, app->game.ghost.row, &I_ghost2); }
...
static void check_for_collisions(InputEvent* event, App* app) {
	...
	if (((app->game.kc.col + (KC_WIDTH /2) >= app->game.ghost.col) && (app->game.kc.col <= app->game.ghost.col + GHOST_WIDTH) && app->game.kc.direction == RIGHT) ||
...
static void character_movement(InputEvent* event, App* app) {
	...
	if (app->game.kc.direction != STOP) {
		...
		if (app->game.ghost.col < GHOST_WIDTH) { app->game.ghost.direction = RIGHT; }
		if (app->game.ghost.col > MAX_COLS - GHOST_WIDTH) { app->game.ghost.direction = LEFT; }
		// walkabout
...
static void kcline_update_timer_callback(FuriMessageQueue *event_queue)
{
...
int32_t kcline_main(void* p) {
    ...
    while(app->scene != SCENE_QUIT) {
		...
		if (event_status == FuriStatusOk && (event.type == InputTypeShort || event.type == EVENT_TYPE_TICK)){
			...
			if (event.type == EVENT_TYPE_TICK) { 
				...
				if (app->scene == SCENE_GAME) {
					...
					if ((count_multiples(app->game.score, 30) > app->game.level) && (app->ticks > TICK_ADVANCE)) {
