#include "flipenigma_file.h"
#include <storage/storage.h>
#include <loader/loader.h>

// #define FLIPENIGMA_APP_BASE_FOLDER APP_BOARDA_PATH("flipenigma")
#define FLIPENIGMA_APP_BASE_FOLDER EXT_PATH("apps_data/flipenigma")
#define FLIPENIGMA_APP_BASE_FOLDER_PATH(path) FLIPENIGMA_APP_BASE_FOLDER "/" path
#define FLIPENIGMA_BOARD_FILE_NAME "board_fen.txt"
#define FLIPENIGMA_BOARD_FILE_NAME_BAK "board_fen.bak"
#define FLIPENIGMA_BOARD_PATH FLIPENIGMA_APP_BASE_FOLDER_PATH(FLIPENIGMA_BOARD_FILE_NAME)
#define FLIPENIGMA_BOARD_PATH_BAK FLIPENIGMA_APP_BASE_FOLDER_PATH(FLIPENIGMA_BOARD_FILE_NAME_BAK)
...
bool flipenigma_has_file(const FlipEnigmaFile file_type, const char* file_name, const bool remove) {
    ...
    if(file_type == FlipEnigmaFileBoard) {
        path = FLIPENIGMA_BOARD_PATH;
    } else {
        strcpy(path_buf, FLIPENIGMA_APP_BASE_FOLDER); // 22
...
bool flipenigma_save_file(
    ...
    if(file_type == FlipEnigmaFileBoard) {
        path = FLIPENIGMA_BOARD_PATH;
        path_bak = FLIPENIGMA_BOARD_PATH_BAK;
    } else {
        strcpy(path_buf, FLIPENIGMA_APP_BASE_FOLDER); // 22
        strcpy(path_buf + strlen(path_buf), "/");
        strcpy(path_buf + strlen(path_buf), file_name);
    ...
    storage_simply_mkdir(fs_api, FLIPENIGMA_APP_BASE_FOLDER);
