...
static void displaySubmenu(UART_TerminalApp *app, UART_TerminalItem *item) {
    ...
    scene_manager_set_scene_state(
        app->scene_manager, UART_TerminalSceneTargets, app->selected_menu_items[GRAVITY_MENU_TARGETS]);
...
static void uart_terminal_scene_targets_var_list_change_callback(VariableItem* item) {
    ...
    const UART_TerminalItem* menu_item = &targets[app->selected_menu_items[GRAVITY_MENU_TARGETS]];
    uint8_t item_index = variable_item_get_current_value_index(item);
    furi_assert(item_index < menu_item->num_options_menu);
    variable_item_set_current_value_text(item, menu_item->options_menu[item_index]);
    app->selected_menu_options[GRAVITY_MENU_TARGETS][app->selected_menu_items[GRAVITY_MENU_TARGETS]] = item_index;
}
...
void uart_terminal_scene_targets_on_enter(void* context) {
    ...
    for(int i = 0; i < NUM_TARGET_ITEMS; ++i) {
        item = variable_item_list_add(
            ...
            app);
        variable_item_set_current_value_index(item, app->selected_menu_options[GRAVITY_MENU_TARGETS][i]);
        variable_item_set_current_value_text(
            item, targets[i].options_menu[app->selected_menu_options[GRAVITY_MENU_TARGETS][i]]);
    }
    variable_item_list_set_selected_item(
        var_item_list, scene_manager_get_scene_state(app->scene_manager, UART_TerminalSceneTargets));
...
bool uart_terminal_scene_targets_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        scene_manager_set_scene_state(app->scene_manager, UART_TerminalSceneTargets, app->selected_menu_items[GRAVITY_MENU_TARGETS]);
        scene_manager_next_scene(app->scene_manager, nextScene);
        consumed = true;
    } else if(event.type == SceneManagerEventTypeTick) {
        app->selected_menu_items[GRAVITY_MENU_TARGETS] = variable_item_list_get_selected_item_index(app->targets_menu_list);
        consumed = true;
    }
    return consumed;
}
