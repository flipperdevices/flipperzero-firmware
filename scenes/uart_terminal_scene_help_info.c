...
static void uart_terminal_scene_help_info_var_list_change_callback(VariableItem* item) {
    ...
    const UART_TerminalItem* menu_item = &help_info[app->selected_menu_items[GRAVITY_MENU_HELP_INFO]];
    uint8_t item_index = variable_item_get_current_value_index(item);
    furi_assert(item_index < menu_item->num_options_menu);
    variable_item_set_current_value_text(item, menu_item->options_menu[item_index]);
    app->selected_menu_options[GRAVITY_MENU_HELP_INFO][app->selected_menu_items[GRAVITY_MENU_HELP_INFO]] = item_index;
}
...
void uart_terminal_scene_help_info_on_enter(void* context) {
    ...
    for(int i = 0; i < NUM_HELP_INFO_ITEMS; ++i) {
        item = variable_item_list_add(
            ...
            app);
        variable_item_set_current_value_index(item, app->selected_menu_options[GRAVITY_MENU_HELP_INFO][i]);
...
bool uart_terminal_scene_help_info_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        app->selected_menu_items[GRAVITY_MENU_HELP_INFO] = variable_item_list_get_selected_item_index(app->help_info_menu_list);
        consumed = true;
    }
    return consumed;
}
