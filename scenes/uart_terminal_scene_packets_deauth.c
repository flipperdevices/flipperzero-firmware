...
static void uart_terminal_scene_packets_deauth_var_list_enter_callback(void* context, uint32_t index) {
    ...
    if (index < DEAUTH_MENU_RUN) {
        ...
        app->selected_menu_options[GRAVITY_MENU_PACKETS_DEAUTH][index] = selected_option_index;
        // YAGNI: Null check
    ...
    if (!strcmp(item->actual_commands[selected_option_index], "")) {
        ...
        for (int i = 0; i < DEAUTH_MENU_RUN; ++i) {
            ...
            cmdLength += strlen(thisItem->actual_commands[app->selected_menu_options[GRAVITY_MENU_PACKETS_DEAUTH][i]]);
        }
        ...
        for (int i = 0; i < DEAUTH_MENU_RUN; ++i) {
            ...
            strcat(deauth_command, packets_deauth[i].actual_commands[app->selected_menu_options[GRAVITY_MENU_PACKETS_DEAUTH][i]]);
        }
...
static void uart_terminal_scene_packets_deauth_var_list_change_callback(VariableItem* item) {
    ...
    const UART_TerminalItem* menu_item = &packets_deauth[app->selected_menu_items[GRAVITY_MENU_PACKETS_DEAUTH]];
    uint8_t item_index = variable_item_get_current_value_index(item);
    furi_assert(item_index < menu_item->num_options_menu);
    variable_item_set_current_value_text(item, menu_item->options_menu[item_index]);
    app->selected_menu_options[GRAVITY_MENU_PACKETS_DEAUTH][app->selected_menu_items[GRAVITY_MENU_PACKETS_DEAUTH]] = item_index;
}
...
void uart_terminal_scene_packets_deauth_on_enter(void* context) {
    ...
    for(int i = 0; i < NUM_PACKETS_DEAUTH_ITEMS; ++i) {
        ...
        variable_item_set_current_value_index(item, app->selected_menu_options[GRAVITY_MENU_PACKETS_DEAUTH][i]);
...
bool uart_terminal_scene_packets_deauth_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        app->selected_menu_items[GRAVITY_MENU_PACKETS_DEAUTH] = variable_item_list_get_selected_item_index(app->packets_deauth_menu_list);
        consumed = true;
    }
    return consumed;
}
