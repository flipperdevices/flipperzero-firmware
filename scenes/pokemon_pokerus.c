...
static void select_pokerus_rebuild_list(PokemonFap* pokemon_fap) {
    ...
    pokerus.days = variable_item_list_add(
        pokemon_fap->variable_item_list, "Days remain:", (strain == 0 ? 0 : 16), select_days_callback, pokemon_fap);
     ...
     */
    if (strain == 0) ;
    else if (((strain & 0x03) == 0) && ((strain & 0xc0) != 0)) strain = 0x01; // This would be A
    else {
        strain &= 0x03;
	strain++;
    }
