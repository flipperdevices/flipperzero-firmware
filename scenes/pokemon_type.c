...
static void select_type_callback(VariableItem* item) {
    ...
    variable_item_set_current_value_text(item, namedlist_name_get_pos(context->pokemon_fap->pdata->type_list, pos));
    pokemon_stat_set(context->pokemon_fap->pdata, STAT_TYPE, context->type, namedlist_index_get(context->pokemon_fap->pdata->type_list, pos));
}
...
void select_type_scene_on_enter(void* context) {
    ...
    for(i = 0; i < 2; i++) {
        ...
        variable_item_set_current_value_text(vitype[i], namedlist_name_get_pos(pokemon_fap->pdata->type_list, pos));
    }
