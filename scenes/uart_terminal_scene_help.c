...
static void displaySubmenu(UART_TerminalApp *app, UART_TerminalItem *item) {
    ...
    scene_manager_set_scene_state(
        app->scene_manager, UART_TerminalSceneHelp, app->selected_menu_items[GRAVITY_MENU_HELP]);
...
static void uart_terminal_scene_help_var_list_change_callback(VariableItem* item) {
    ...
    app->selected_menu_options[GRAVITY_MENU_HELP][app->selected_menu_items[GRAVITY_MENU_HELP]] = item_index;
}
...
void uart_terminal_scene_help_on_enter(void* context) {
    ...
    for(int i = 0; i < NUM_HELP_ITEMS; ++i) {
        item = variable_item_list_add(
            ...
            app);
        variable_item_set_current_value_index(item, app->selected_menu_options[GRAVITY_MENU_HELP][i]);
...
bool uart_terminal_scene_help_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        scene_manager_set_scene_state(app->scene_manager, UART_TerminalSceneHelp, app->selected_menu_items[GRAVITY_MENU_HELP]);
        scene_manager_next_scene(app->scene_manager, nextScene);
        consumed = true;
    } else if(event.type == SceneManagerEventTypeTick) {
        app->selected_menu_items[GRAVITY_MENU_HELP] = variable_item_list_get_selected_item_index(app->help_menu_list);
        consumed = true;
    }
    return consumed;
}
