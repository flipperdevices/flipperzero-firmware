...
static void select_item_selected_callback(void* context, uint32_t index) {
    ...
    FURI_LOG_D(
        TAG,
        "[item] Set item %s",
        namedlist_name_get_index(pokemon_fap->pdata->item_list, pokemon_stat_get(pokemon_fap->pdata, STAT_HELD_ITEM, item)));
...
void select_item_set_scene_on_enter(void* context) {
    ...
    for(i = 1;; i++) {
        name = namedlist_name_get_pos(pokemon_fap->pdata->item_list, i);
        if(name == NULL) break;
        if(name[0] == letter && (pokemon_fap->pdata->gen & namedlist_gen_get_pos(pokemon_fap->pdata->item_list, i))) {
            submenu_add_item(
                pokemon_fap->submenu,
		name,
