...
static FuriString* path_parse(FuriString* path){
	...
	bool is_root;
	if(furi_string_cmp_str(path, "") == 0) is_root = false;
	else is_root = furi_string_get_char(path, 0) == '/';
	...
	for(int i = 0; i < PATH_SPLIT_SIZE; i++){
		...
		if(furi_string_cmp_str(parts[i], "..") == 0){
			...
			int j = i - 1;
			while(j != -1){
				if(furi_string_cmp_str(parts[j], "") != 0 && furi_string_cmp_str(parts[j], ".") != 0 && furi_string_cmp_str(parts[j], "..") != 0){
...
static void path_split(FuriString* src, FuriString* path1, FuriString* path2){
	...
	if(index == 4294967295) furi_string_set_str(path2, "");
	else furi_string_u_substr(path2, index + 1, furi_string_size(src));
...
static void write_handler(Cli* cli, FuriString* args, void* context){
	...
	uint16_t bytes_written = storage_file_write(file, furi_string_get_cstr(content), furi_string_size(content));
...
static void start_handler(Cli* cli, FuriString* args, void* context){
	...
	do {
		...
		for(int i = 0; i < read_size; i++){
			...
			else furi_string_push_back(line, content[i]);
		}
	} while(read_size > 0);
	furi_string_set(lines[cur_line], line);
