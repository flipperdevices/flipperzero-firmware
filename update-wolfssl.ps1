...
try {
    ...
    $wolfssl_exclude_paths = "^wolfSSL-wolfssl-\w+\/(wolfcrypt\/(test\/|benchmark\/|src\/port\/|src\/.+\.i$|src\/.+\.S$|src\/.+\.asm$|src\/.+\.am$|user-crypto\/)|(wolfssl\/(test\.h$|openssl\/|wolfcrypt\/port\/|.+\.am$|.+\.rc$)))"
    try {
