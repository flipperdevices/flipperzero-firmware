...
def prepare_git_targets(cmd_environment):
    ...
    git_sm_update = git_environment.Command(
        "build/submodule_update.flag",
        Value(get_fast_git_version_id()),
        [
            Action("git submodule update --init"),
            Touch("$TARGET"),
        ],
    )
    git_environment.Alias("submodule_update", git_sm_update)


def prepare_fw_targets(cmd_environment):
...
if GetOption("git-tasks"):
    prepare_git_targets(cmd_environment)
else:
    prepare_fw_targets(cmd_environment)
