...
int flipperzero_dcf77_app_main(void *p)
{
  ...
  app_data.sec = app_data.dt.second;
  ...
  while (running) {
    if (silence) {
      furi_hal_rfid_tim_read_start(77500, 1);
      furi_hal_pwm_start(FuriHalPwmOutputIdLptim2PA4, 77500, 50);
      furi_hal_light_set(LightBlue, 0xFF);
      silence = false;
    }
    furi_hal_rtc_get_datetime(&app_data.dt);
    if (app_data.dt.second != app_data.sec) {
      app_data.sec = app_data.dt.second;
      if (app_data.sec < 59) {
        ...
        silence = true;
        wait_ms = get_dcf77_bit(app_data.sec) ? 200 : 100;
      }
      else {
        uint32_t next_minute = furi_hal_rtc_datetime_to_timestamp(&app_data.dt) + 61;

        FuriHalRtcDateTime dcf_dt;
        furi_hal_rtc_timestamp_to_datetime(next_minute, &dcf_dt);
