        ...
        if(app->state->waitForCode && recieved > 0)
        {
            ...
            for(int i = 0; i < l; i++)
            {
                ...
                { // reset for timeout and continue :(
                    ...
                    if((app->state->rpackets == 0 && app->state->spackets == 0) || (assert_next_colon && out[i] != ':'))
                    {
                ...
                if(app->state->codeLen > 0 && app->state->rpackets == app->state->codeLen && app->state->spackets == app->state->codeLen)
                {
                    ...
                    furi_string_set_strn(app->dialog_header, furi_string_get_cstr(app->state->s_code), MAX_DISPLAY_BYTES);
                    if(furi_string_size(app->dialog_header) >= MAX_DISPLAY_BYTES)
                        furi_string_cat(app->dialog_header, "...");
                    dialog_ex_set_header(app->dialog, furi_string_get_cstr(app->dialog_header), 64, 12, AlignCenter, AlignTop);
...
void fcom_read_code_scene_on_enter(void* context) {
    ...
    bool hasData = !furi_string_empty(app->state->r_code) && !furi_string_empty(app->state->s_code);
    ...
    else
    {
        ...
        dialog_ex_set_text(app->dialog, "Connect to device bus and initiate comms", 10, 24, AlignLeft, AlignTop);
