...
void gameplay_selection_set_to_valid_cell(AppGameplayState* gameplay)
{
    ...
    if (nextBoard != -1)
    {
        ...
        {
            int cellIndex = (4 + i) % 9;
            if (gameplay_selection_try_set_to_cell(gameplay, nextBoard, cellIndex))
                return;
        }
    ...
    if (gameplay_selection_try_set_to_cell(gameplay, 4, 4))
        return;

    for (int i = 0; i < 9; i++)
    {
        if (gameplay_selection_try_set_to_cell(gameplay, 4, i))
            return;
    }
    ...
    for (int i = 0; i < 9; i++)
    {
        if (gameplay_selection_try_set_to_cell(gameplay, i, 4))
            return;
    }
    ...
    for (int i = 0; i < 9; i++)
    {
        for (int j = 0; j < 9; j++)
        {
            if (gameplay_selection_try_set_to_cell(gameplay, i, j))
                return;
        }
    }
...
void game_selection_reset_for_next_player(AppGameplayState* gameplay)
{
    ...
    if (gameplay_get_player_type(gameplay, game_get_player_turn(gameplay->game)) == PlayerType_Human)
        gameplay_selection_set_to_valid_cell(gameplay);
}

void find_next_empty_cell(AppGameplayState* gameplay, int baseX, int baseY, int dx, int dy, int fromX, int toX, int fromY, int toY, int* outBoardIndex, int* outCellIndex)
{
...
void gameplay_selection_handle_delta(AppGameplayState* gameplay, int dx, int dy)
{
    ...
    find_next_empty_cell(gameplay, selectionX, selectionY, dx, dy, minCellX, maxCellX, minCellY, maxCellY, &resultBoardIndex, &resultCellIndex);
