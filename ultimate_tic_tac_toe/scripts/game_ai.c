...
void game_ai_get_movement_minimax(GameState* game, int* outBoardIndex, int* outCellIndex, int* outScore, int depth)
{
    const int WinnerScore = 100000;

    BoardWinner myWinner = game_get_player_turn(game) == PlayerTurn_X ? BoardWinner_X : BoardWinner_O;
    ...
    for (int boardIndex = 0; boardIndex < 9 && !winFound; boardIndex++)
    {
        for (int cellIndex = 0; cellIndex < 9 && !winFound; cellIndex++)
        {
            ...
            if (game_get_next_board(gameCopy) == -1)
                score -= 100;

            score += cellIndex % 2 == 0 ? rand() % 95 : rand() % 85; // Randomize ties. Slightly favor the center and the corners.
...
{
    if (game_get_winner(gameplay_get_game(gameplay)) != BoardWinner_TBD)
        return;
    ...
    if (timeSinceLastMovement > TimeThinking && (selectionBoardIndex == -1 || selectionCellIndex == -1))
    {
        ...
        {
            int _;
            game_ai_get_movement_minimax(game, &selectionBoardIndex, &selectionCellIndex, &_, game_ai_get_depth(playerType));
        }
    ...
    if (timeSinceLastMovement > TimeMoving && (selectionBoardIndex != -1 && selectionCellIndex != -1))
    {
