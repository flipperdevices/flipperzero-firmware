...
void game_try_set_board_winner(GameState* game, int boardIndex, int a, int b, int c)
{
    ...
    if (game->board[boardIndex][a] != CellState_Empty && game->board[boardIndex][a] == game->board[boardIndex][b] && game->board[boardIndex][a] == game->board[boardIndex][c])
...
void game_try_set_winner(GameState* game, int a, int b, int c)
{
    ...
    if (game->boardWinner[a] == BoardWinner_TBD || game->boardWinner[a] == BoardWinner_Draw)
        return;

    if (game->boardWinner[a] == game->boardWinner[b] && game->boardWinner[a] == game->boardWinner[c])
...
void game_update_board_winner(GameState* game, int boardIndex)
{
    ...
    if (game->boardWinner[boardIndex] == BoardWinner_X || game->boardWinner[boardIndex] == BoardWinner_O)
    {
    ...
    if (game->boardWinner[boardIndex] == BoardWinner_TBD)
    {
        ...
        if (filledCells == 9)
            game->boardWinner[boardIndex] = BoardWinner_Draw;
    }

    // If the board was finished, check if the game is over
    if (game->boardWinner[boardIndex] != BoardWinner_TBD)
        game_update_winner(game);
}
