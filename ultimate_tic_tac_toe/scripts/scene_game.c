...
void draw_single_board(Canvas* const canvas, GameState* game, int boardIndex, int x, int y, bool isNextBoard, int selectedCell)
{
    ...
    for (int cellIndex = 0; cellIndex < 9; cellIndex++)
    {
        ...
        {
            canvas_draw_line(canvas, cellPositionX, cellPositionY, cellPositionX + 2, cellPositionY + 2);
            canvas_draw_line(canvas, cellPositionX, cellPositionY + 2, cellPositionX + 2, cellPositionY);
        }
...
void draw_board(Canvas* const canvas, AppGameplayState* gameplay)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        bool isNextBoard = game_get_winner(game) == BoardWinner_TBD && (game_get_next_board(game) == -1 || game_get_next_board(game) == i);
        int selectedCell = game_get_winner(game) == BoardWinner_TBD && i == selectionBoardIndex ? selectionCellIndex : -1;
...
void draw_status(Canvas* const canvas, GameState* game)
{
    ...
    if (game_get_winner(game) == BoardWinner_X)
    {
        for (int i = -3; i <= 3; i++)
        {
            canvas_draw_line(canvas, statusX - 20 + i, statusY - 20, statusX + 20 + i, statusY + 20);
            canvas_draw_line(canvas, statusX - 20 + i, statusY + 20, statusX + 20 + i, statusY - 20);
        }
    }
...
void game_handle_input(InputKey key, InputType type, void* context)
{
    ...
    if (game_get_winner(gameplay_get_game(app->gameplay)) != BoardWinner_TBD)
        return;

    if (gameplay_get_next_player_type(app->gameplay) != PlayerType_Human)
        return;
    ...
    if (key == InputKeyLeft)
        gameplay_selection_handle_delta(app->gameplay, -1, 0);

    if (key == InputKeyRight)
        gameplay_selection_handle_delta(app->gameplay, 1, 0);

    if (key == InputKeyUp)
        gameplay_selection_handle_delta(app->gameplay, 0, -1);
