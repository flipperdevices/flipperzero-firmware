...
printf "echo >/dev/null # >nul & @echo off & GOTO WINDOWS & rem ^$WIN_END" > "$OUT_FILE"
printf "# ***********************************************************$UNIX_END" >> "$OUT_FILE"
printf "# * NOTE: Generated by scripts/toolchain/generate-fbt.sh$UNIX_END" >> "$OUT_FILE"
...
printf "    scripts/toolchain/unix-toolchain-download.sh || exit 1;$UNIX_END" >> "$OUT_FILE"
...
printf "set \"toolchainRoot=%%~dp0toolchain\"$WIN_END" >> "$OUT_FILE"
printf "set \"SCONS_DEFAULT_FLAGS=-Q --warn=target-not-built\"$WIN_END" >> "$OUT_FILE"
printf "if not exist \"%%toolchainRoot%%\" (powershell -ExecutionPolicy Bypass -File %%~dp0scripts\\\toolchain\\\windows-toolchain-download.ps1)$WIN_END" >> "$OUT_FILE"
printf "cmd /V /C \"set \"PATH=%%toolchainRoot%%\\\python;%%toolchainRoot%%\\\bin;%%toolchainRoot%%\\\protoc\\\bin;%%PATH%%\" && python lib\\\scons\\\scripts\\\scons.py %%SCONS_DEFAULT_FLAGS%% %%*\"$WIN_END" >> "$OUT_FILE"
