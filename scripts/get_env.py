...
def add_envs(data, gh_env_file, args):
    ...
    add_set_output_var("branch_name", data["branch_name"], gh_output_file)
    add_set_output_var("commit_sha", data["commit_sha"], gh_output_file)
    add_set_output_var("default_target", os.getenv("DEFAULT_TARGET"), gh_output_file)
    add_set_output_var("suffix", data["suffix"], gh_output_file)
    if args.type == "pull":
