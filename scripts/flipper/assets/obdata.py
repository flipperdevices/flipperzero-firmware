...
class OptionByte:
    ...
    def encode(self):
        ...
        value_shifted = self.value << startbit
        value_mask_shifted = value_mask << startbit
        value_inverted_masked = (value_shifted ^ 0xFFFFFFFF) & value_mask_shifted
        return EncodedOBValue(value_shifted, value_mask_shifted, value_inverted_masked, self)
...
class ObReferenceValuesGenerator:
    ...
    def __repr__(self):
        return f"<OBRefs REFS=[{' '.join(hex(v) for v in self.ref_values)}] " \
    ...
    def export_values(self):
        ...
        #print("VALUES", export_refvals.tobytes())
...
def main():
    ...
    #if len(ob_sample_arr) != cmp_vals
    for idx in range(len(ob_sample_arr)):
        real_masked = ob_sample_arr[idx] & mask[idx]
        print(f"#{idx}: ref {ref[idx]:08x} real {real_masked:08x} ({ob_sample_arr[idx]:08x} & {mask[idx]:08x}) match {ref[idx]==real_masked}")
