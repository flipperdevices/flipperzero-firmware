...
class TestEmulateClassic:
    ...
    def add_4k7_manually(self, nav):
        # Code below should be run once before the tests

        # !! Disabled for tests with real cards, re-enable when needed !!
        yield
        return
        # !!
    ...
    @pytest.mark.parametrize("condition, cmds", tests_data)
    def test_trailer_access_condition(self, px, emulate_classic, condition, cmds):
        for i, cmd in enumerate(cmds):
            ...
            # 6 retries seems to be enough for most of the cases with unreliable communication
            retry_count = 6
