...
class TestRfidBench(BaseCase):
    def test_read_indala_card(self, nav, gator, reader_indala):
        ...
        start_time = time.time()
        state = nav.get_current_state()
        while ("status_Writing Indala26" in state) or ("warn_Still trying to write" in state):
            state = nav.get_current_state()
    ...
    def test_read_em_card(self, nav, gator, reader_em_hid):
        ...
        start_time = time.time()
        state = nav.get_current_state()
        while ("status_Writing EM4100" in state) or ("warn_Still trying to write" in state):
            state = nav.get_current_state()
            if time.time() - start_time > 25:
                break
        state = nav.get_current_state()
    ...
    def test_read_hid_card(self, nav, gator, reader_em_hid):
        ...
        start_time = time.time()
        state = nav.get_current_state()
        while ("status_Writing H10301" in state) or ("warn_Still trying to write" in state):
            state = nav.get_current_state()
            if time.time() - start_time > 25:
                break
        state = nav.get_current_state()
