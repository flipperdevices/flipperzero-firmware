...
class TestInfraredBench(BaseCase):
    def test_new_remote_and_universal_remotes(self, nav_reader, nav_key):
        ...
        with allure.step("Learn new remote on reader_flipper"):
            ...
            nav_reader.press_ok()
            nav_key.press_ok()
            assert nav_reader.wait_for_state("SaveRight") == 0, "Infrared reading error while adding new remote"
            nav_reader.press_right()
            nav_reader.press_ok()
            assert nav_reader.wait_for_state("ir_SIRC_15_button") == 0, "Error while saving first button in IR remote"
        ...
        with allure.step("Adding another button in remote on reader_flipper"):
            ...
            assert "ir_add_button" in nav_reader.get_current_state(), "Error while starting adding new button to remote"
            nav_reader.press_ok()
            nav_key.press_back()
            ...
            nav_reader.press_up()
            assert nav_reader.wait_for_state("ir_NECe_ED12_button") == 0, "Error while saving second button in IR remote"
            if "status_ir_sending" in nav_key.get_current_state():
                nav_key.press_back()
        with allure.step("Adding another button in remote on reader_flipper"):
            nav_reader.press_down()
            ...
            nav_reader.wait_for_state("ir_add_button")
            nav_reader.press_up()
            assert nav_reader.wait_for_state("ir_NEC_8A_button") == 0, "Error while adding third button in IR remote"
            if "status_ir_sending" in nav_key.get_current_state():
                nav_key.press_back()
        with allure.step("Adding another button in remote on reader_flipper"):
            nav_reader.press_down()
            ...
            nav_reader.wait_for_state("ir_add_button")
            nav_reader.press_up()
            assert nav_reader.wait_for_state("ir_RAW_109_button") == 0, "Error while adding last button in IR remote"
            if "status_ir_sending" in nav_key.get_current_state():
                nav_key.press_back()
    ...
    def test_debug(self, nav_reader, nav_key):
        ...
        with allure.step("Check transaction"):
            ...
            nav_reader.press_ok()
            assert "ir_NEC_8A_bench" in nav_key.get_current_state(), "Error while recieving test signal"