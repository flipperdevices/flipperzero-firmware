...
uint32_t furi_thread_enumerate(FuriThreadList* thread_list) {
    uint32_t i, count, tick;
    TaskStatus_t* task;

    if(FURI_IS_IRQ_MODE() || (thread_list == NULL)) {
        tick = 0U;
    } else {
        vTaskSuspendAll();
        tick = furi_get_tick();
        ...
        if(task) {
            vPortFree(task);
            furi_thread_list_cleanup(thread_list, total_run_time, tick);
        }
    }
