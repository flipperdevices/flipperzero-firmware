...
static void showStartupScreen()
{
    ...
    const char *tempMessage="Toggle all squares to be bright.\nSome squares are connected.\ngithub.com/nostrumuva";
...
static void gameToggleSquare(Game *paramGame,int paramSquareIndex)
{
    ...
    for(tempIterateConnected=0;tempIterateConnected<BOARD_NUM_SQUARES;tempIterateConnected++)
    {
        if(paramGame->myBoard[paramSquareIndex].myConnectedSquares[tempIterateConnected]==1&&tempIterateConnected!=paramSquareIndex)
        {
            paramGame->myBoard[tempIterateConnected].mySquareActive=(SquareActive)(!((bool)paramGame->myBoard[tempIterateConnected].mySquareActive));
        }
    }
    paramGame->myBoard[paramSquareIndex].mySquareActive=(SquareActive)(!((bool)paramGame->myBoard[paramSquareIndex].mySquareActive));
}
...
static void render_callback(Canvas* const canvas,void *ctx)
{
    ...
    furi_string_printf(tempStatusStr,"L: %d/%d   T: %d",tempGame->myLevel,MAX_LEVELS,tempGame->myNumToggles);
    canvas_set_font(canvas,FontSecondary);
    canvas_draw_str_aligned(canvas,0,0,AlignLeft,AlignTop,furi_string_get_cstr(tempStatusStr));
    static uint8_t staticMinX=40;
    static uint8_t staticMinY=8;
    static uint8_t staticSquareActiveOnBits[]={0x00,0x7E,0x42,0x42,0x42,0x42,0x7E,0x00,};//1 is black, 0 is bright
    static uint8_t staticSquareActiveOnBitsSelected[]={0xFF,0xFF,0xC3,0xC3,0xC3,0xC3,0xFF,0xFF,};//1 is black, 0 is bright
    static uint8_t staticSquareActiveOffBits[]={0x00,0x7E,0x7E,0x7E,0x7E,0x7E,0x7E,0x00,};//1 is black, 0 is bright
    static uint8_t staticSquareActiveOffBitsSelected[]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,};//1 is black, 0 is bright
    int tempIterateSquares=0;
    int tempSquareX=0;
    int tempSquareY=0;
    for(tempIterateSquares=0;tempIterateSquares<BOARD_NUM_SQUARES;tempIterateSquares++)
...
int32_t secret_toggle_app(void* p)
{
    ...
    while(tempContinueGame==true)
    {
        ...
        {
            FuriStatus tempFuriStatus=furi_message_queue_get(newFuriMessageQueue,&tempPluginEvent,100);
