...
{  
  ...
  if (scan_mode == WIFI_SCAN_OFF || scan_mode == WIFI_SCAN_JSON_OFF)
    StopScan(scan_mode);
  ...
  else if (scan_mode == WIFI_SCAN_TARGET_JSON_AP)
   RunAPScanJSON(scan_mode, color);
...
// Function to start running a beacon scan
void WiFiScan::RunAPScanJSON(uint8_t scan_mode, uint16_t color)
{
  esp_wifi_init(&cfg);
  esp_wifi_set_storage(WIFI_STORAGE_RAM);
  esp_wifi_set_mode(WIFI_MODE_NULL);
  ...
  esp_wifi_set_promiscuous_filter(&filt);
  //if (scan_mode == WIFI_SCAN_TARGET_AP_FULL)
  esp_wifi_set_promiscuous_rx_cb(&apSnifferJSONCallback);
  //else
  //  esp_wifi_set_promiscuous_rx_cb(&apSnifferCallback);
  esp_wifi_set_channel(set_channel, WIFI_SECOND_CHAN_NONE);
  this->wifi_initialized = true;
...
void WiFiScan::apSnifferJSONCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      if (!in_list) {
        ...
        // Serial.print("RSSI: ");
        // Serial.print(snifferPacket->rx_ctrl.rssi);
        wifiJSON["rssi"] = snifferPacket->rx_ctrl.rssi;
        // Serial.print(" Ch: ");
        // Serial.print(snifferPacket->rx_ctrl.channel);
        wifiJSON["ch"] = snifferPacket->rx_ctrl.channel;
        ...
        wifiJSON["essid"] = essid;
        wifiJSON["free_heap"] = esp_get_free_heap_size();
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  (currentScanMode == WIFI_SCAN_STATION) ||
  (currentScanMode == WIFI_SCAN_SIG_STREN) ||
  (currentScanMode == WIFI_SCAN_TARGET_JSON_AP) ||
  (currentScanMode == WIFI_SCAN_TARGET_AP) ||
  (currentScanMode == WIFI_SCAN_PWN) ||
  (currentScanMode == WIFI_SCAN_DEAUTH) ||
  (currentScanMode == WIFI_SCAN_ALL))
