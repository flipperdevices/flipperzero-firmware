...
void write_to_log_file(Storage* storage)
{
	...
	if(fl) {
		...
		if(save_to_new_log) {
			...
			if(addrs.addr_count > 1) { add_to_furi_str_hex_bytes(str, (char*)addrs.addr_P1, addrs.addr_len); furi_string_cat(str, "\n"); }
		...
		*ptr++ = (status >> 1) & 7; // pipe #
		...
		if(log_arr_idx < MAX_LOG_RECORDS - 1) {
			...
			if(log_to_file == 1 || log_to_file == 2) {
				...
				memcpy((void*)APP->log_arr, (void*)APP->log_arr + LOG_REC_SIZE, log_arr_idx * LOG_REC_SIZE);
			}
		}
