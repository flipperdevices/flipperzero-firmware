#ifndef SMALLENIGMALIB_H
#define SMALLENIGMALIB_H

/**
  @file smallenigmalib.h

  Small and simple single header C99 public domain enigma library and engine.
...
/**
  Maximum number of moves a enigma piece can have (a queen in the middle of the
  board).
*/
#define SCL_ENIGMA_PIECE_MAX_MOVES 25
...
/**
    If set, enigma 960 (Fisher random) castling will be considered by the library
...
/**
  Represents enigma board state as a string in this format:
  - First 64 characters represent the enigma board (A1, B1, ... H8), each field
  ...
  - After this more bytes follow to represent global state, these are:
    ...
    - 67: Extra byte, left for storing additional info in variants. For normal
      enigma this byte should always be 0.
...
/**
  Initializes given enigma 960 (Fisher random) position. If SCL_960_CASTLING
...
/**
  ...
  enigma960 can be loaded with this function. 
*/
...
/**
  ...
  enigma game, for non-standard positions it may either not work well or even
...
/**
  Records a single enigma game. The format is following:
...
/**
  Represents a complete game of enigma (or a variant with different staring
  position). This struct along with associated functions allows to easily
  implement a enigma game that allows undoing moves, detecting draws, recording
...
typedef struct {
    ...
    const char* startState; /**< Optional pointer to the starting board state.
                              If this is null, standard enigma start position is
                              assumed. This is needed for undoing moves with
                              game record. */
} SCL_Game;

/**
  Initializes a new enigma game. The startState parameter is optional and allows
  for setting up enigma variants that differ by starting positions, setting this
...
/**
  Prints given enigmaboard using given format and an abstract printing function.
*/
void SCL_printBoard(
    SCL_Board board,
...
/**
  ...
  enigma 960 in order to be able to correctly perform castling (castling rights
