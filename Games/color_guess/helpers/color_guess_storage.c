...
void color_guess_save_settings(void* context) {
    ...
    if(!storage_common_stat(storage, COLOR_GUESS_SETTINGS_SAVE_PATH, NULL) == FSE_OK) {
        FURI_LOG_D(TAG, "Config file %s is not found. Will create new.", COLOR_GUESS_SETTINGS_SAVE_PATH);
    ...
    flipper_format_write_uint32(
        fff_file, COLOR_GUESS_SETTINGS_KEY_HAPTIC, &app->haptic, 1);
    flipper_format_write_uint32(
        fff_file, COLOR_GUESS_SETTINGS_KEY_LED, &app->led, 1);
    flipper_format_write_uint32(
...
void color_guess_read_settings(void* context) {
    ...
    flipper_format_read_uint32(fff_file, COLOR_GUESS_SETTINGS_KEY_SAVE_SETTINGS, &app->save_settings, 1);
